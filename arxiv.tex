...
\begin{abstract}
	...
	To reap the benefits of RIScatter, we also propose a co-located \gls{sic}-free receiver that semi-coherently decodes the backscatter information, recovers the reflection pattern, and coherently decodes the primary link.
	For a single-user multi-node RIScatter network, we characterize the achievable primary-(total-)backscatter rate region by designing the input distribution at scatter nodes, the active beamforming at the \gls{ap}, and the energy decision regions at the user.
...
\begin{IEEEkeywords}
	Input distribution design, symbol-level precoding, index modulation, active-passive coexisting network, \gls{sic}-free receiver, energy detection, backscatter communication, reconfigurable intelligent surface, ambient backscatter communication, symbiotic radio.
\end{IEEEkeywords}
...
\begin{section}{Introduction}
	...
	\begin{enumerate}
		...
		\item The spreading factor (i.e., backscatter symbol duration over primary) is usually large\footnote{The load-switching interval of low-power backscatter modulators is usually {0.1} to \SI{10}{\us} \cite{Torres2021}, accounting for a typical spreading factor between $10$ and $10^3$.}.
	\end{enumerate}
	...
	The concept is shown in Fig.~\subref*{fg:riscatter} where one or more RIScatter nodes ride over an active transmission and simultaneously modulate their information and engineer the wireless channel.
...
\begin{section}{RIScatter}
	\begin{subsection}{Principles}
		...
		% can be described by a unified signal model and implemented by passive antennas or programmable metamaterial \cite{Liang2022}.
		...
		% A natural question is, is it possible to use a scatterer
		...
		\begin{figure}[!t]
			...
			\caption{
				...
				In this example, the optimal passive beamformer is state {2} and its input probability for RIScatter is {0.75}.
				% RIScatter node transmit symbol {1}
				The spreading factor is {4} for RIScatter and {8} for \gls{ambc}/\gls{sr}.
			}
		...
		\begin{remark}
			...
			% The latter only involves time sharing between different states but there are many ways to ensure within a channel block.
			...
			% However, different combinations correspond to different backscatter messages and RIScatter impact is determined dynamic passive beamforming once successfully decoded.
		\end{remark}
		...
		% We also propose a practical co-located receiver to exploit the benefits of RIScatter and the detail will be covered in Section~\ref{se:system_model}.
	\end{subsection}

	% As illustrated in Fig.~\ref{fg:energy_distribution}, conditioned on different reflection state hypotheses, the accumulated energy per backscatter block follows Gamma distribution with different scale parameters \cite{Qian2017b}.
	% \begin{figure}[!t]
	% 	\centering
	% 	\resizebox{0.8\columnwidth}{!}{
	...
	% 		$z$, $t$, $\mathcal{H}$ and $\mathcal{R}$ denote the accumulated energy, decision threshold, reflection state hypothesis, and decision regions, respectively.
	% 	}
	% 	\label{fg:energy_distribution}
	% \end{figure}

	% Hence, the receiver can semi-coherently (under primary uncertainty) decode RIScatter nodes from the accumulated energy, re-encode to recover the reflection patterns, and combine those with cascaded \gls{csi} to determine the multipath response and model within the primary channel.
	...
	\begin{subsection}{System Model}
		...
		% For dispersed RIScatter nodes, relevant \gls{csi} can be estimated by sequential \cite{Bharadia2015,Yang2015b,Guo2019g} or parallel approaches \cite{Jin2021a} originally proposed for \gls{bc}. For co-located RIScatter nodes, the estimation can be simplified by group-based \cite{Zheng2019} and hierarchical \cite{You2019} trainings originally proposed for \gls{ris}.}.
		...
		\end{equation}
		% The receiver first jointly decodes all backscatter messages under primary interference by energy detection.
		% This essentially formulates a \gls{dmmac} over disjoint energy decision regions.
		...
		% Conditioned on different reflection state hypotheses, the accumulated energy per backscatter block follows Gamma distribution with different scale parameters.
		\begin{figure}[!t]
			\centering
			% \resizebox{0.8\columnwidth}{!}{
			% 	\input{assets/illustration/energy_distribution.tex}
			% }
			\includegraphics[width=0.8\columnwidth]{assets/illustration/energy_distribution.jpg}
			\caption{
				\gls{pdf} of accumulated energy per backscatter block conditioned on different reflection state hypotheses.
			}
			\label{fg:energy_distribution}
...
\begin{section}{Simulation Results}
	...
	% The \gls{ap} is with an average transmit power budget $P=\SI{36}{\dBm}$, all nodes employs $M$-\gls{qam} with amplitude scattering ratio $\alpha=0.5$, and the user is with average noise power $\sigma_v^2=\SI{-40}{\dBm}$.
	...
	We set $b=9$ and $\varepsilon={e-3}$.
	All achievable rate points/regions are averaged over $10^3$ channel realizations.
	...
	\begin{subsection}{Evaluation of Proposed Algorithms}
		...
		\begin{subsubsection}{Convergence}
			...
			For $K=8$ and $M=2$, Algorithm \ref{al:input_distribution} typically takes around {100} fast iterations to converge to the \gls{kkt} input distribution.
			For $Q=4$, around {10} iterations are required for Algorithm \ref{al:active_beamforming} to converge.
			Overall, the initial \gls{bcd} algorithm requires at most {5} iterations to converge.
	...
	\begin{subsection}{Comparison of Scattering Applications}
		...
		\begin{itemize}
			...
			% The corresponding \gls{pdf} of accumulated energy over $N$ primary blocks is
			% \begin{equation}
			...
			% This essentially formulates a \gls{dcmc} and the total backscatter achievable rate is maximized with equiprobable inputs as \cite{Ng2006}
			% \begin{equation}
		...
		\begin{figure}[!t]
			...
			\caption{Typical achievable rate region/points of scattering applications for $Q=1$, $K=1$, $M=4$, $N=10^3$, $\sigma_v^2=\SI{-40}{\dBm}$ and $r=\SI{2}{\meter}$.}
		...
		% \emph{First,} we observe both \gls{bbc} and \gls{sr} achieves when the spreading factor $N$ is sufficiently large.
		\emph{First,} we observe \gls{bbc} and \gls{sr} achieve the best backscatter performance thanks to coherent decoding.
		% However, \gls{sr} requires $N$ \gls{sic} followed by a \gls
		% However, \gls{sr} requires $N$ re-encoding, precoding, subtraction together with a time-domain \gls{mrc} to decode each backscatter symbol and the cost can be prohibitive.
		...
		% However, the primary rate benefit
		% For \gls{bbc} with coherent energy detection, the conditional \gls{pdf} of accumulated energy \eqref{eq:energy_distribution_bbc} is more skewed at a large $N$, such that the equivalent \gls{dmmac} \eqref{eq:dmmac} has lower error probabilities.
	...
	\begin{subsection}{Input Distribution under Different \gls{qos}}
		...
		At $\rho=0$ where the backscatter performance is prioritized, the optimal input distribution is {0} on two states and nearly uniform on the other two.
		...
		At $\rho=1$ where the primary performance is prioritized, the optimal input distribution is {1} at the state that maximizes the primary \gls{snr} and {0} at the others.
		That is, the reflection pattern becomes deterministic and the RIScatter node boils down to a discrete \gls{ris} element.
		Increasing $\rho$ from {0} to {1} provides a smooth transition from backscatter modulation to passive beamforming and proves RIScatter unifies \gls{bc} and \gls{ris} from a probabilistic perspective.
	\end{subsection}
	...
	\begin{subsection}{Rate Region by Different Schemes}
		...
		\begin{subsubsection}{Input Distribution}
			...
			\begin{itemize}
				...
				\item \emph{Exhaustion:} Exhaustive search over the $M$-dimensional probability simplex with resolution $\Delta p = {e-2}$;
			...
			\begin{itemize}
				...
				\item \emph{Decomposition:} Normalized rank-{1} \gls{cp} decomposition tensors by \texttt{Tensor Toolbox} \cite{Bader2022};
	...
	\begin{subsection}{Rate Region under Different Configurations}
		...
		\begin{subsubsection}{Number of States}
			...
			Due to the maximum amplitude normalization in \eqref{eq:backscatter_modulation}, the average rate region of {8}-\gls{qam} (points on rectangle border) is smaller than that of {4}-\gls{qam} (points on square vertices), and the {4} points on the rectangle edges are less frequently used.
			On the other hand, {16}-\gls{qam} provides significant higher backscatter rate benefit than primary.
		...
		\begin{subsubsection}{Spreading factor}
			Fig.~\subref*{fg:region_duration} presents how the spreading factor $N$ affects the achievable rate region.
			...
			As $N \to \infty$, RIScatter nodes boil down to \gls{ris} elements with fixed reflection pattern during whole channel block and the total backscatter rate approaches {0}.
...
\begin{appendix}
	...
	% 	Since $L$ input letters are with non-zero probability and $x \to z \to \hat{x}$ formulates a Markov chain, we need $L$ non-empty decision regions and at least $L+1$ distinct thresholds (including {0} and $\infty$) to minimize the distortion between source and decision.
