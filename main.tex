...
\begin{section}{Simulation Results}
	...
	\begin{subsection}{Comparison of Scattering Applications}
		Based on the setup in Fig. \ref{fg:riscatter_network}, we compare the achievable rates of RIScatter for different applications:
		\begin{itemize}
		...
		The simulation results are shown in Fig. \ref{fg:region_comparison}.
		First, we observe both \gls{bbc} and \gls{sr} almost achieve noise-free backscatter transmission when $N$ is sufficiently large.
		For \gls{bbc} using coherent energy detection, the conditional \gls{pdf} of accumulated receive energy \eqref{eq:energy_distribution_bbc} is more skewed at a large $N$, such that the equivalent \gls{dmtmac} \eqref{eq:dmtmac} becomes more reliable.
		...
		However, such an \gls{sr} requires $N$ \gls{sic} processes to detect one backscatter symbol (tuple), and the semi-coherent primary rate benefit only appears at a very large $N$.
		When $N$ is moderate, the randomly scattered signals should be modelled as primary interference rather than quasi-static multipath components, and the \gls{sr} point will move towards the \gls{ambc} point.
		% \footnote{\gls{sr} is essentially a multiplicative \gls{noma} that cancels interference, while \gls{ambc} is essentially a multiplicative \gls{sdma} that treats interference as noise.}
		% Besides, the deterministic reflection pattern of \gls{ris} provides constructive primary signal superposition
		% \gls{ris} achieves the highest primary achievable rate using deterministic reflection pattern.
		% Second, the passive beamforming gain by a single \gls{ris} node
		% Second, the average primary achievable rate decreases in the presence of a \gls{ambc} node and subtly/notably increases in the presence of a \gls{sr}/\gls{ris} node.
		Second, the average primary rate slightly increases/decreases in the presence of a \gls{ambc}/\gls{ris} node, while the multipath benefit of \gls{sr} is indistinguishable for a small $K$.
		This is because the cascaded channel strength can be orders of magnitude lower than the direct channel strength due to the double fading effect.
		\gls{ris} always ensures constructive superposition of direct and scattered signal components, while \gls{sr} only creates a rich-scattering environment that stochastically enhances the average primary rate.

		Third, RIScatter enables flexible
		% adapt the reflection state distribution of passive scatter devices based on \gls{csi} and link weights, in order to flexibly transition between backscatter modulation and passive beamforming.



		% the inter \gls{ambc} node decreases

		In comparison, RIScatter performs semi-coherent backscatter detection and coherent primary detection to enable backscatter modulation and passive beamforming for arbitrary $N$, and the primary-backscatter tradeoff can be controlled by \gls{csi}- and weight-adaptive input distribution design.

		% , and different rate points can be achieved by different node input distributions.

		% coherent primary detection, and the
