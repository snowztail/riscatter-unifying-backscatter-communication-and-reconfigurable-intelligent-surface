...
\begin{document}
	...
	\begin{section}{System Model}
		...
		To investigate how backscatter modulation potentially benefits the primary transmission, we first decode the tag symbol $c$ in presence of unknown AP symbol $\boldsymbol{s}$. Interestingly, the tag detection indeed aligns with the case of Ambient Backscatter Communications (AmBC) where the signal from the ambient source is unknown. The optimal Maximum-Likelihood (ML) detector is
		\begin{equation}
			\hat{m} = \arg \max_{m \in \mathcal{M}} p(\boldsymbol{y} \mid c_m),
		\end{equation}
		where $p(\boldsymbol{y} \mid c_m)$ denotes the probability density function of receiving $\boldsymbol{y}$ when the tag transmits $c_m$.

		During the backscatter detection, the conditional probability density function of the channel output given the tag input for $M$-PSK is
		\begin{equation}
			f_{Y \mid C}(y \mid c_m) = ?
		\end{equation}

		% Since $\mathbb{E}[\lvert s \rvert^2] = \mathbb{E}[\lvert c \rvert^2] = 1$, by treating the primary interference as noise, the SINR for the user to decode tag message $c$ is
		% \begin{equation}
