...
\begin{section}{Backscatter Principles}
	...
	\begin{subsection}{Metascatter: Bridge and Generalization}
		\begin{figure}[!t]
			...
			\caption{
				Input probability distribution and time block structure of \gls{sr}, \gls{irs}, and Metascatter.
...
\begin{section}{Simulation Results}
	...
	We assume the distance between \gls{ap} and user is \SI{10}{\meter}, and $K=2$ Metascatters are uniformly dropped within a disk of radius \SI{1}{\meter} centered at the user.
	% The carrier frequency is $f=\SI{200}{\MHz}$, and we consider \gls{iid} Ricean fading for all channels.
	The carrier frequency is $f=\SI{200}{\MHz}$, and we consider \gls{iid} Ricean fading between all terminals.
	For direct, forward and backward links, the path loss exponents are \num{2.6}, \num{2.4} and \num{2}, while the Ricean factor are \num{5}, \num{5} and \num{10}, respectively.
	...
	The \gls{ap} has $Q=4$ antennas with maximum average transmit power $P=\SI{36}{\dBm}$.
	All node perform \gls{qam} with $M=2$ input states, the amplitude reflect ratio is $\alpha=0.5$, and the symbol period ratio is $N=20$.
	% and we consider amplitude reflect ratio $\alpha=0.5$ and symbol period ratio $N=20$.
	% The user has a receive antenna gain of \SI{3}{\dBi} and average noise power
	The user is with average noise power $\sigma_v^2=\SI{70}{\dBm}$ and receive antenna gain \SI{3}{\dBi}.
	All rate regions are averaged over \num{1000} instances.
	% Each node has $M=2$ candidate input states with amplitude reflect ratio $\alpha=0.5$.
	...
	\begin{figure}[!t]
		...
		\caption{Input distribution vs weight $\rho$.}
	\end{figure}
	...
	% 	}
	% 	\caption{1}
	% 	% \caption{Average primary-(total-)backscatter rate region.}
	% \end{figure}
	% \begin{table}[t!]
	% 	\centering
	...
	% 		\includegraphics{assets/illustration/rate_region_1.eps}
	% 	}
	...
	% 	\centering
	% 	\resizebox{\columnwidth}{!}{
	% 		\includegraphics{assets/illustration/rate_region_2.eps}
	% 	}
