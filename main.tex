	...
	\begin{section}{Input Distribution, Decision Region, and Precoder Design}
		...
		\begin{subsection}{Input probability distribution}
			...
			\begin{proposition}
				...
				% An interpretation is that each probable state of each tag should produce the same marginal information (averaged over all states of other tags), while any state of any tag with potentially less marginal information than above should not be used.
				% \begin{itemize}
				% 	\item each probable state of each tag produces the same marginal information (averaged over all states of other tags)
				% 	\item all states of all tags with potentially less marginal information should be dropped
				% \end{itemize}
				% An input probability distribution maximizes the weighted sum primary-backscatter rate if and only if each probable state of each tag produces the same mutual information. Specifically, the optimal $\boldsymbol{P}^{\star}$ to problem~\eqref{op:input_probability_distribution}

				% the input probability distribution that achieves the weighted sum primary-backscatter capacity, at state $i_q \in \mathcal{M}$ of tag $q \in \mathcal{K}$,
				% \begin{subequations}
				% 	\begin{alignat}{2}
			...
			\begin{proof}
				...
				% , and we define $\forall i_q \in \mathcal{M}$ and $\forall q \in \mathcal{K}$
				\begin{align}
					S_k'(\bar{c}_{m_q})
				...
				TODO
				% \begin{equation}
				% 	P_q^{\star}(\bar{c}_{i_q}) = \frac{P_q^{(r)}(\bar{c}_{i_q}) \exp \left( \frac{\rho}{1 - \rho} I_q(\bar{c}_{i_q};\boldsymbol{y},z) \right)}{\sum_{i_q} P_q^{(r)}(\bar{c}_{i_q}) \exp \left( \frac{\rho}{1 - \rho} I_q(\bar{c}_{i_q};\boldsymbol{y},z) \right)}.
				% 	\label{eq:optimal_reference_distribution}
				% \end{equation}


				% of choosing any $i_q$ coincide
				% Regularization on \eqref{eq:optimal_intermediate_information_condition}
			\end{proof}
