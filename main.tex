...
\begin{document}
	...
	\begin{section}{The Multi-Tag Case}
		...
		\begin{subsection}{Backscatter Detection}
			...
			\end{equation}
			which denotes the expectation of the received power per primary block under tag modulation index set $m_{\mathcal{K}}$.
			% It can be numerically evaluated based on the precoder, tag, channel, and noise knowledge.
			For the ease of exposition, we denote the hypothesis that the tag input combination at status $i \in \mathcal{M^K} \triangleq \{1,\ldots,M^K\}$ as $\mathcal{H}_i$, sort $\{\sigma_{m_{\mathcal{K}}}^2\}$ in ascending order by a one-to-one mapping $m_{\mathcal{K}} \mapsto i$,\footnote{When more than one modulation index sets yield the same energy level, the mapping is not unique and the detection fails to separate them. This blind spot issue can be mitigated by multi-antenna techniques.} define the received signal energy over $N$ primary blocks as $z \triangleq \lVert \boldsymbol{y} \rVert^2$, and let $f(z \mid \mathcal{H}_i)$ be the conditional probability density function of receiving $z$ under hypothesis $\mathcal{H}_i$. Correspondingly, $z$ is the sum of $N$ i.i.d. exponential variables each with mean $\sigma_i^2$, and the conditional probability density function follows Erlang distribution with shape $N$ and scale $\sigma_i^2$ as
			\begin{equation}
			...
			\end{equation}
			where the detection threshold between $\mathcal{H}_i$ and $\mathcal{H}_{i'}$ is
			\begin{equation}
		...
		\begin{subsection}{Backscatter sum-rate}
			...
			\begin{equation}
				I_{\mathrm{B},\mathcal{S}}(\bar{c}_{m_{\mathcal{S}}} ; z) = \sum_{m_{\mathcal{K} \setminus \mathcal{S}}} P_{\mathcal{K} \setminus \mathcal{S}}(\bar{c}_{m_{\mathcal{K} \setminus \mathcal{S}}}) I(\bar{c}_{m_{\mathcal{K}}} ; z),
			...
			\begin{equation}
				I_{\mathrm{B}}(c_{\mathcal{K}} ; z) = \mathbb{E}_{c_{\mathcal{K}}} \left[I_{\mathrm{B}}(\bar{c}_{m_{\mathcal{K}}} ; z)\right] = \mathbb{E}_{c_{\mathcal{S}}} \left[I_{\mathrm{B},\mathcal{S}}(\bar{c}_{m_{\mathcal{S}}} ; z)\right].
				\label{eq:mutual_information}
			\end{equation}

			Evidently, for a given discrete memoryless MAC with transition probability $P(z \mid c_{\mathcal{K}})$, the information function $I(\bar{c}_{m_{\mathcal{K}}} ; z)$ and mutual information $I(c_{\mathcal{K}} ; z)$ only depend on the tag input distribution $P_k(\bar{c}_{m_k})$, $\forall m_k \in \mathcal{M}$, $\forall k \in \mathcal{K}$. Therefore, the mutual information of the tags can be explicitly written as
			\begin{equation}
				I_{\mathrm{B}}(c_{\mathcal{K}} ; z) = \sum_{m_{\mathcal{K}}} \prod_{k \in \mathcal{K}} P_k(\bar{c}_{m_k}) \sum_j P(\bar{z}_j \mid \bar{c}_{m_{\mathcal{K}}}) \log \frac{P(\bar{z}_j \mid \bar{c}_{m_{\mathcal{K}}})}{P(\bar{z}_j)}.
				\label{eq:backscatter_sum_rate}
			\end{equation}

			% we can explicitly write the mutual information as

			% obtain the backscatter sum-rate by maximizing the mutual information over the tag input distributions as
			% \begin{equation}
		...
		\begin{subsection}{Primary ergodic rate}
			Once the tag symbol set $\bar{c}_{m_{\mathcal{K}}}$ is successfully decoded, the user can eliminate the backscatter uncertainty and model it within the equivalent primary channel by \eqref{eq:equivalent_primary_channel}. For primary transmission, the tags adjust the propagation environment in a potentially beneficial manner, and create artificial channel variation within each fading block. As such, the ergodic primary mutual information can be expressed as
			\begin{align}
				I_{\mathrm{P}}
	...
	\begin{section}{Input Distribution, Decision Region, and Precoder Design}
		\begin{subsection}{Primary ergodic-secondary sum rate region}
			...
			\begin{maxi!}
				{\scriptstyle{P(c_{\mathcal{K}}),\mathcal{T},\boldsymbol{w}}}{I \triangleq \rho I^{\mathrm{P}} + (1 - \rho) I^{\mathrm{B}}}{\label{op:rate_region}}{\label{ob:weighted_sum_rate}}
		...
		\begin{subsection}{Input distribution}
			Once the decision boundaries $\mathcal{T}$ and the precoder $\boldsymbol{w}$ are determined, obtain the equivalent discrete memoryless MAC by \eqref{eq:mac}. The input distribution optimization subproblem is
			\begin{maxi!}
				{\scriptstyle{P(c_{\mathcal{K}})}}{I}{\label{op:input_distribution}}{\label{ob:weighted_sum_rate_input_distribution}}
				\addConstraint{\sum_{m_k} P_k(\bar{c}_{m_k})}{=1,\quad\label{co:sum_probability_input_distribution}}{\forall k \in \mathcal{K}}
				\addConstraint{P_k(\bar{c}_{m_k})}{\ge 0,\quad\label{co:nonnegative_probability_input_distribution}}{\forall m_k \in \mathcal{M}, \ \forall k \in \mathcal{K}}
			\end{maxi!}

			Although the domain is non-convex for $K>1$, the \gls{kkt} conditions are sufficient and necessary for an input distribution to achieve the weighted sum-capacity. The proof follows straightforwardly from \cite{Watanabe2009} and details are omitted here.\footnote{For any elementary MAC (the sizes of input alphabets are no greater than that of the output alphabet), the sufficiency can be proved by local maximum and connectedness of \gls{kkt} solutions. Also, the capacity of an arbitrary MAC is achieved by an elementary MAC included within.}
			% We restrict the discussion of this paper to elementary MACs.
