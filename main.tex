...
\title{RIScatter: Unifying Backscatter Communications and Reconfigurable Intelligent Surface From a Probabilistic Perspective}
...
% * Wi-Fi sampling rate 20 MHz (symbol period 50 nS), RFID symbol rate/load switching speed 100s KHz to 10s MHz [Backscatter Communications], fast load switching speed is key for high data rate - symbol ratio is not very large
...
\begin{abstract}
	...
	RIScatter is a new paradigm for future wireless networks and consists of multiple dispersed or co-located passive scatter nodes, whose reflection states can be adapted to partially engineer the wireless channel and partially modulate information onto the scattered wave.
	...
	Simulation results demonstrate RIScatter nodes can exploit the additional propagation paths to smoothly transition between backscatter modulation and passive beamforming via smart input distribution design, and the proposed receiver significantly reduces the decoding complexity and effectively accommodates the double modulation and signal difference in active-passive coexisting network.
\end{abstract}
...
\begin{section}{Introduction}
	\begin{subsection}{Fundamentals}
		...
		To tackle this, \cite{Liu2013b} proposed an \gls{ambc} system in Fig. \subref*{fg:ambc}, where battery-free nodes recycle ambient legacy signals (e.g., radio, television and Wi-Fi) to harvest energy and establish connections in between.
	...
	\begin{table*}[!t]
		...
		\begin{tabularx}{\textwidth}{CCCCCCC}
			...
			Coexisting systems            & 1                         & 1                         & 2 (competitive)                                 & 2 (collaborative)                            & 1                                          & 2 (collaborative)                                 \\
			\rowcolor{gray!25}
			Scatterer contribution        & Backscatter modulation    & Backscatter modulation    & Backscatter modulation and primary interference & Backscatter modulation and primary multipath & Passive beamforming                        & Backscatter modulation and passive beamforming    \\
			...
			\rowcolor{gray!25}
			Backscatter detection         & Coherent                  & Coherent                  & Semi-coherent or noncoherent                    & Coherent                                     & ---                                        & Semi-coherent                                     \\
			Reflection state distribution & Equiprobable (line-coded) & Equiprobable (line-coded) & Equiprobable (line-coded)                       & Equiprobable or Gaussian                     & Degenerate (\gls{csi}-adaptive)            & Flexible (\gls{csi}- and traffic-adaptive)        \\
			\rowcolor{gray!25}
	...
	\begin{subsection}{Contributions}
		As presented in Fig. \subref*{fg:riscatter}, we propose RIScatter as a novel scattering protocol that generalizes \gls{bc}, \gls{sr}, and \gls{ris}.
...
\begin{section}{Scattering Principles}
	...
	\gls{bc}, \gls{ambc} and \gls{sr} employ scatter nodes as information sources that encode message by \emph{probabilistically switching} between different states.
...
\begin{section}{Simulation Results}
	...
	\begin{subsection}{Evaluation of Proposed Algorithms}
		...
		\begin{subsubsection}{Convergence}
			\begin{figure}[!t]
				...
				\caption{Typical convergence curves at $\rho=0$ for $Q=4$, $K=8$, $M=2$, $N=20$ and $\sigma_v^2=\qty{-40}{\dBm}$.}
			...
			For $K=8$ and $M=2$, Algorithm \ref{al:input_distribution} typically takes around \num{100} fast closed-form iterations by \eqref{eq:input_kkt_solution} to converge to the \gls{kkt} input distribution.
			...
			Hence, we conclude the proposed algorithms are able to converge within moderate iterations and provide significant rate benefits for RIScatter systems.
		\end{subsubsection}
	\end{subsection}
	...
	\begin{subsection}{Comparison of Scattering Applications}
		...
		\begin{itemize}
			...
			The \gls{ml} decision threshold is consequently derived as, $\forall l \in \mathcal{L} \setminus \{L\}$,
			\begin{equation}
			...
			During backscatter detection, the primary symbols $s[1],\ldots,s[n]$ can be viewed as a spreading code, and the receiver can perform \gls{mrc} over $N$ primary blocks.
		...
		\begin{figure}[!t]
			...
			\caption{Typical achievable rate region/points of scattering applications for $Q=1$, $K=1$, $M=4$ and $N=1000$.}
		...
		Such an \gls{sr} requires a sufficiently large $N$ to guarantee the primary performance, but decreases the backscatter symbol rate by a factor of $N$ and requires $N$ \gls{sic} processes to decode each backscatter symbol (tuple).
		\emph{Second,} the average primary rate slightly increases/decreases in the presence of a \gls{ambc}/\gls{ris} node, but the multipath benefit of \gls{sr} is unobvious.
		...
		\gls{ris} always ensures constructive superposition of direct and scattered components, but \gls{sr} only creates a quasi-static rich-scattering environment that stochastically enhances the average primary rate.
		...
		This is because both RIScatter and \gls{ambc} employ semi-coherent (instead of coherent) energy detection in the presence of primary uncertainty, and RIScatter with adaptive channel coding achieves higher backscatter rate than \gls{ambc} with equiprobable inputs.
	...
	\begin{subsection}{Input Distribution under Different Weights}
		\begin{figure}[!t]
			...
			\caption{Typical RIScatter reflection state distribution under different weights for $Q=1$, $K=1$, $M=4$ and $N=20$.}
		...
		In such cases, the extreme states producing the lowest/highest energy are always assigned with non-zero probability, while the others may not provide enough energy difference and end up unused.
	...
	\begin{subsection}{Rate Region by Different Schemes}
		\begin{figure}[!t]
			...
			\caption{
				Average primary-total-backscatter rate regions by different input distribution, active beamforming, and decision threshold schemes for $K=2$, $M=4$ and $N=20$.
			}
		\end{figure}
		\begin{subsubsection}{Input Distribution}
			...
			\begin{itemize}
				\item \emph{Cooperation:} Joint encoding with a $K$-dimensional probability array $P_{\mathcal{K}}(x_{m_{\mathcal{K}}})$ by Algorithm \ref{al:input_distribution};
			...
			\end{itemize}
			Since scatter cooperation is unavailable for dispersed nodes, we also propose these methods to recover input distributions (for independent encoding) from the joint probability array:
			\begin{itemize}
				\item \emph{Marginalization:} Marginal probability distributions;
				\item \emph{Decomposition:} Normalized rank-\num{1} \gls{cp} decomposition tensors \cite{Bader2022};
			...
			The average rate performance of \emph{Exhaustion} and \emph{\gls{kkt}} coincide with each other, which verifies Remark \ref{re:input_kkt_distribution} that \gls{kkt} input distribution can be reasonably good for a moderate $K$.
		...
		\begin{subsubsection}{Active Beamforming}
			...
			In the low-$\rho$ regime, the proposed \emph{\gls{pgd}} beamformer significantly outperforms both \gls{mrt} schemes in terms of backscatter rate.
			...
			Such an energy diversity is effectively enhanced by \emph{\gls{pgd}} that exploits backscatter constellation and input distribution knowledge, rather than simply maximizing the direct/ergodic equivalent \gls{snr}.
			...
			At $\rho=1$, \emph{\gls{pgd}} and \emph{E-\gls{mrt}:} boil down to \gls{mrt} towards the deterministic primary equivalent channel as commonly considered in \gls{ris} literatures.
			Besides, the difference between \emph{E-\gls{mrt}} and \emph{D-\gls{mrt}} can be insignificant for dispersed RIScatter nodes.
			Those observations prove the proposed \emph{\gls{pgd}} active beamforming design can flexibly improve primary equivalent \gls{snr} and enhance backscatter energy difference to enlarge RIScatter achievable rate region.
		\end{subsubsection}
