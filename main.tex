...
\begin{section}{Introduction}
	...
	The achievable rate and optimal input distribution for binary-input \gls{ambc} are investigated in \cite{Qian2019b}, but its impact on the primary link is not considered.
	...
	In contrast, \cite{Vardakis2023} exploits commodity \gls{rfid} tags, powered and controlled by a software-defined radio reader, to perform passive beamforming (but no backscatter modulation) towards a legacy user at a different frequency. \label{cm:2.5}
	...
	The concept is shown in Fig.~\subref*{fg:riscatter} where one or more RIScatter nodes ride over an active transmission, simultaneously modulating their information and engineering the wireless channel.
	...
	Thanks to double modulation, the backscatter detection becomes part of channel training, and the impact of backscatter modulation can be modelled as dynamic passive beamforming.
	The proposed receiver may be built over legacy receivers with minor hardware upgrade, since it only requires one additional energy comparison and re-encoding per backscatter symbol (instead of primary symbol).
	...
	This is the first paper to propose a \gls{sic}-free receiver for active-passive coexisting networks.
	...
	Uniquely, our optimization problem takes into account the \gls{csi}, \gls{qos}, and backscatter constellation, and the resulting input distribution is applicable to any detection scheme.
...
\begin{section}{RIScatter}
	...
	\begin{subsection}{Principles}
		...
		Its impact is usually modelled as part of environment multipath \cite{Thomas2012,Liang2020}, or simply a \gls{dc} offset in the baseband signal when the impinging signal is a \gls{cw} \cite{Boyer2014}.
		...
		RIScatter nodes can be implemented, for example, by adding an integrated receiver\footnote{The aim is to coordinate the node with the active source and to acquire the optimized input distribution, instead of decoding the primary information dedicated for the user. The node receiver can be implemented using very simple circuits or even integrated with the rectifier \cite{Kim2021a} to reduce cost and complexity.\label{fn:integrated_receiver}} \cite{Kim2021a} and adaptive encoder \cite{He2020e} to off-the-shelf passive \gls{rfid} tags.
	...
	\begin{subsection}{System Model}
		...
		Once the backscatter information is successfully decoded, the user re-encodes to recover the exact reflection pattern and thus the composite channel by \eqref{eq:equivalent_channel}, then coherently decodes the primary link.
...
\begin{section}{Rate-Region Characterization}
	...
	To obtain the achievable primary-(total-)backscatter rate region, we consider the weighted sum mutual information maximization problem with independent encoding at all nodes\footnote{Joint encoding over multiple nodes can be viewed as its special case with an augmented source.\label{fn:joint_encoding}}.
	\begin{maxi!}
...
\begin{section}{Simulation Results}
	...
	In this section, we provide numerical results to evaluate the proposed input distribution, active beamforming, and backscatter decision algorithms.
	...
	The \gls{ap} is with an average transmit power budget $P=\qty{36}{dBm}$ and all nodes employs $M$-\gls{qam} with $\alpha=0.5$.
	...
	\begin{subsection}{Comparison of Scattering Applications}
		...
		Here, \gls{ris} ensures constructive superposition of direct and scattered components while \gls{sr} only creates a quasi-static rich-scattering environment that marginally enhances the average primary rate.
		...
		This is because both RIScatter and \gls{ambc} employ semi-coherent energy detection, and the adaptive channel coding of RIScatter outperforms the equiprobable inputs of \gls{ambc}, especially at the low backscatter \gls{snr} in the double fading scenario.
		When multiple antenna is available at the \gls{ap}, active beamforming can be optimized for RIScatter nodes and the backscatter rate gain over \gls{ambc} is more prominent.
	\end{subsection}
	...
	\begin{subsection}{Input Distribution under Different \gls{qos}}
		...
		This is inline with Shannon's observation that binary antipodal inputs is good enough for channel capacity at the low \gls{snr} \cite{Shannon1948}.
		...
		The extreme states producing the lowest/highest energy are always assigned with non-zero probability, while the middles cannot provide enough energy difference and thus end up unused.
		At $\rho=1$ where the primary link is prioritized, the optimal input distribution is \num{1} at state \num{3}, since the corresponding reflection coefficient maximizes the primary \gls{snr}.
		That is, the reflection pattern becomes deterministic and the RIScatter node boils down to a discrete \gls{ris} element.
		Increasing $\rho$ from \num{0} to \num{1} provides a smooth transition from backscatter modulation to passive beamforming, suggesting RIScatter unifies \gls{bc} and \gls{ris} from a probabilistic perspective.
	\end{subsection}
	...
	\begin{subsection}{Rate Region by Different Schemes}
		...
		\begin{subsubsection}{Input Distribution}
			...
			\begin{itemize}
				...
				\item \emph{\gls{kkt}:} The \gls{kkt} solution by Algorithm \ref{al:input_distribution};
			...
			It is observed that Cooperation achieves the outer bound of all schemes, but its backscatter rate gain comes at the cost of implementation and computation complexity.
			Besides, the average rate performance of Exhaustion and \gls{kkt} completely coincide with each other when $K=2$.
			This confirms Remark \ref{re:input_kkt_distribution} that the \gls{kkt} input distribution can be good enough when $K$ is not very large.
			Equiprobable experiences minor backscatter and major primary rate losses without exploiting \gls{csi} and \gls{qos}.
			Those gaps should be larger when $M$ and/or $K$ increase.
			For the recovery methods, Marginalization provides a close performance to \gls{kkt}, but Randomization and Decomposition fail our expectations for most channel realizations.
		...
		\begin{subsubsection}{Active Beamforming}
			...
			This is because the semi-coherent backscatter decoding relies on the \emph{relative difference} of receive energy under different backscatter symbol tuples.
			Such an energy diversity is enhanced by \gls{pga}, which effectively exploits backscatter constellation and input distribution knowledge rather than simply maximizing the direct/ergodic channel strength.
			As $\rho$ increases, the primary \gls{snr} outweighs the backscatter energy difference in \eqref{eq:weighted_sum_mutual_information_explicit}, and \gls{pga} beamformer approaches both \gls{mrt} schemes.
			...
			Those observations prove the proposed \gls{pga} active beamforming design can exploit the \gls{csi}, \gls{qos}, and backscatter constellation to enlarge the achievable rate region.
		\end{subsubsection}
	...
	\begin{subsection}{Rate Region under Different Configurations}
		...
		\end{figure}
		In this study, we choose $Q=4$, $K=8$, $M=2$, $N=20$, $\sigma_v^2=\qty{-40}{dBm}$ and $r=\qty{2}{m}$ as a reference.
		\begin{subsubsection}{Number of Nodes}
			...
			On the other hand, when the primary link is prioritized, the RIScatter nodes boil down to \gls{ris} elements and enjoy a conventional passive array gain $K^2$.
		\end{subsubsection}

		\begin{subsubsection}{Number of States}
			Fig.~\subref*{fg:region_states} shows the relationship between available reflection states (i.e., \gls{qam} order) $M$ and achievable rate regions when $K=1$.
			...
			It greatly enhances the receive energy difference but cannot provide enough phase shift resolution with strong reflection.
		\end{subsubsection}
		...
		\begin{subsubsection}{Number of Transmit Antennas}
			...
			As $Q$ increases, more scattered paths are available to \gls{pga} and the channel diversity can be further exploited to enhance the primary \gls{snr} and backscatter energy difference.
		...
		\begin{subsubsection}{Spreading factor}
			Fig.~\subref*{fg:region_duration} shows how the spreading factor $N$ affects the achievable rate region.\footnote{Please notice in Fig.~\subref*{fg:region_duration} the unit of total backscatter rate is bits per primary block to indicate throughput.}
			Using a very large $N$ can severely constrain the backscatter throughput, since the gain in energy certainty (by law of large numbers) cannot withstand the loss in gross rate.
			As $N \to \infty$, RIScatter nodes boil down to static \gls{ris} elements and the total backscatter rate approaches \num{0}.
			On the other hand, when $N$ is too small, energy detection is error-prone as the \gls{dmmac} \eqref{eq:dmmac} is unreliable.
			It explains the observation that $N=10$ provides lower backscatter throughput than $N=20$.
			Therefore, we conclude the spreading factor $N$ should be carefully designed over multiple factors (e.g., data rate requirements, load switching speed at the nodes, and signal processing capability at the user).
		\end{subsubsection}
		...
		\begin{subsubsection}{Average Noise Power}
			...
			When $\sigma_v^2$ relatively high, one can choose a larger $N$ to improve the energy detection \gls{snr}.
		\end{subsubsection}
		...
		\begin{subsubsection}{Imperfect \gls{csi}}
			...
			This is because the energy detector is sensitive to the \gls{dmmac} \eqref{eq:dmmac} and thus $\hat{\boldsymbol{h}}_{\text{C},k}$.
		...
		\begin{subsubsection}{Primary \gls{snr}}
			...
			The backscatter \gls{snr} is fixed to \qty{0}{dB} in this study.
			...
			% This, together with Fig.~\subref*{fg:region_beamforming} and \subref*{fg:region_tags}, emphasizes the importance of balancing the primary and backscatter \gls{snr} in active-passive coexisting networks.
			This, together with Fig.~\subref*{fg:region_beamforming} and \subref*{fg:region_tags}, emphasizes the importance of accounting the primary-backscatter tradeoff in active-passive coexisting networks.
		\end{subsubsection}

		\begin{subsubsection}{Backscatter \gls{snr}}
			The primary \gls{snr} is fixed to \qty{20}{dB} in this study.
