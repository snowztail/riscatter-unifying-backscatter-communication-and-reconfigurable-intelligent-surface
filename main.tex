	...
	\begin{section}{Backscatter Detection and Achievable Rates}
		...
		\begin{subsection}{Backscatter Detection}
			...
			\begin{proof}
				...
				\end{subequations}
				% For \gls{dmtc} based on energy detection~\eqref{eq:energy_distribution}, it is trivial to conclude $t_0^\star = 0$ and $t_S^\star = \infty$.
				...
				% . By Descartes' rule of signs~\cite{Jameson2006}, for each $j \in \mathcal{L}$, $t_s^\star$ has at most 1 real solution if $\sigma_l^2 \ne \sigma_{l'}^2$, $\forall l,l' \in \mathcal{L}$.

				% In other words, based on $t_0^\star = 0$, each decision region $j \in \mathcal{L}$ requires at most one additional threshold, such that $S \le L+1$.
				% $s \equiv j \pmod L$
