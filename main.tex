...
\begin{section}{Simulation Results}
	...
	\begin{subsection}{Comparison of Scattering Applications}
		Based on the setup in Fig. \ref{fg:riscatter_network}, we compare the achievable rates of RIScatter and different scattering applications:
		\begin{itemize}
			\item \emph{\gls{bbc}:}
			...
			Hence, the primary achievable rate can be approximated by\footnote{To provide a preliminary benchmark, we consider the (correlated) scattered signal from finite-input backscatter sources as independent interference from Gaussian sources during primary decoding.}
			\begin{equation}
		...
		Fig. \ref{fg:region_comparison} compares typical achieve rate points of different scattering applications.
		First, we observe both \gls{bbc} and \gls{sr} almost reach the noise-free backscatter rate when $N$ is sufficiently large.
		For \gls{bbc} with coherent energy detection, the processing loss of signal-to-energy conversion can be insignificant, thanks to the noise hardening effect at large $N$.
		For \gls{sr}, \gls{mrc} increases the equivalent backscatter \gls{snr} by $N$ times and the penalty term $\epsilon$ becomes negligible at large $N$.
		However, the backscatter rate benefit requires $N$ \gls{sic} processes per backscatter block, and the primary rate benefit under semi-coherent detection requires a very large $N$.
		When $N$ is moderate, the randomly scattered signals should be modelled as interference rather than quasi-static multipath components during primary decoding, and the \gls{sr} point will move towards \gls{ambc}.
		% Using a very large $N$ not only reduces the system throughput but also requires the operation-intensive \gls{sic} over each primary block.
		% When the primary and backscatter symbol periods are comparable, the randomly scattered signals should be modelled as interference rather than quasi-static multipath components during semi-coherent primary decoding, and the achievable rate point of \gls{sr} will move towards \gls{ambc}.
		% For a moderate $N$, the randomly scattered signals should be modelled as interference rather than quasi-static multipath components during primary decoding, and the \gls{sr} rate point will move towards \gls{ambc}.
