...
\begin{abstract}
	...
	The key principle in RIScatter is to render the probability distribution of reflection states as a joint function of the information source, \gls{csi}, and \gls{qos} of the coexisting active primary and passive backscatter links.
...
\begin{section}{Introduction}
	\begin{subsection}{Fundamentals}
		...
		This concept was further refined by \cite{Liang2020} as \gls{sr} in Fig.~\subref*{fg:sr} that cognitively incorporates \gls{ambc} with existing systems.
		...
		On the other hand, \gls{ris} in Fig.~\ref{fg:ris} is a smart planar signal reflector that consists of numerous compact passive elements with adjustable amplitude and phase responses, whose reflection pattern over time is deterministic and optimized based on \gls{csi}.
		...
		% On the other hand, \gls{ambc} and \gls{sr} involve \emph{coexisting} active primary and passive backscatter links with different relationships in between.
		...
		In \gls{ambc}, the backscatter link only needs to know the basic information (e.g., operating frequency and input distribution) of the primary link, and the primary link does not need to know the existence of the backscatter link.
		% In \gls{sr}, relevant \gls{csi} of both links should be available at the co-located receiver for cooperative decoding
		% In \gls{sr}, the co-located receiver needs to know the \gls{csi} of both links for cooperative decoding, and the primary transmitter may know
		In \gls{sr}, relevant \gls{csi} is exploited at the co-located receiver for cooperative decoding, which may also be exploited at the primary transmitter for cooperative active beamforming.
	...
	% 		Backscatter impact on primary detection         & ---                 & Interference       & Channel uncertainty                           & Passive beamforming & Dynamic passive beamforming                                             \\
	...
	% - sr: rx (possibly tx)
	% - riscatter: rx, tx, sx
	% ! csi, qos and backscatter constellation-aware input distribution + beamforming
	% ! csi and backscatter constellation-aware detection
	...
	\begin{subsection}{Related Works}
		...
		% 1) there exists a \emph{double modulation} where primary and backscatter symbols are superimposed by multiplication coding;
		1) primary and backscatter symbols are superimposed by \emph{double modulation} (i.e., multiplication coding);
		2) backscatter \emph{signal strength} is much weaker than primary because of the double fading effect;
		3) backscatter \emph{symbol period} is longer than primary because of the load switching constraint.
		...
		% During primary decoding, the randomness of backscatter modulation can be modelled as either interference or channel uncertainty, depending on the backscatter-over-primary symbol period ratio $N$\footnote{For state-of-the-art low-power \gls{rfid} tags, the load-switching speed usually varies between \num{100}s of \unit{\kHz} to \num{10}s of \unit{\MHz} \cite{Torres2021}, corresponding to a typical $N$ between \num{10} and \num{1000}.}.
		...
		% , depending on the backscatter-over-primary symbol period ratio $N$\footnote{For state-of-the-art low-power \gls{rfid} tags, the load-switching speed usually varies between \num{100}s of \unit{\kHz} to \num{10}s of \unit{\MHz} \cite{Torres2021}, corresponding to a typical $N$ between \num{10} and \num{1000}.}.
		Such a \gls{sic} receiver is operation-intensive and \gls{csi}-sensitive, since it involves $N$ re-encoding, precoding, subtraction together with a time-domain \gls{mrc} to decode each backscatter symbol.
		...
		Relevant literatures also consider either \emph{Gaussian} codebook \cite{Guo2019b,Ding2020,Long2020a,Zhou2019a,Wu2021a,Xu2021a,Yang2021a,Hu2021b} that is impractical for low-power nodes, or finite \emph{equiprobable} inputs \cite{Yang2018,Liang2020,Han2021,Zhang2022,Liu2019d,Bereyhi2020,Xu2020b,Zhang2021d,Hua2022,Basar2020,Ma2020a,Yuan2021,Hu2021a} that does not fully exploit \gls{csi} and signal properties.
		% For active-passive coexisting networks, it has been observed that:
		% 1) the transmitter-scatter-receiver path can be exploited to adjust wireless channel and establish passive connection;
		% 2) co-located receiver enables interference cancellation and cooperative decoding;
		% 3) the primary and backscatter symbols are superimposed by multiplication (i.e., double modulation).
	\end{subsection}
	...
	\begin{subsection}{Contributions}
		...
		\emph{Second,} we address the first challenge by proposing a practical receiver that effectively accommodates the signal properties of active-passive coexisting networks without \gls{sic}.
		...
		The semi-coherent backscatter detection is indeed part of primary channel estimation.
...
\begin{section}{RIScatter}
	\begin{subsection}{Concepts}
		...
		Next, we propose a cooperative receiver that effectively exploit the signal properties to reduce the decoding complexity and improve the primary-backscatter tradeoff.
