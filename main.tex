...
\begin{section}{Introduction}
	...
	% In \gls{ambc} \cite{Liu2013b}, ambient signals (e.g., radio, television, and Wi-Fi) are recycled by interactive nodes to harvest energy and establish connection.
	...
	% ambient signals (e.g., radio, television, and Wi-Fi) are recycled by interactive nodes to harvest energy and establish connection.
	% It not only eliminates the need of power source and carrier emitter, but also avoid the cost of dedicated frequency spectrum.
	...
	% By extending the \gls{sic} scheme to multi-node scenario, \cite{Xu2021a} proposed a backscatter \gls{noma}-based \gls{sr} where the decoding order depends on backscatter channel strength.
	...
	% Although transmission time and reflection ratio can be designed to boost energy efficiency, it requires frequent feedback to passive nodes and incurs high coordination cost.
	% In \cite{Vougioukas2019}, the nodes control the load-switching speed to shift the scattered signal to desired frequency band, enabling backscatter \gls{fdma}.
	...
	% It requires extra bandwidth and consumes more power at passive nodes.
	% In \cite{Han2021}, random code-assisted backscatter multiple access was introduced to \gls{sr}, and the authors evaluated the asymptotic \gls{sinr} performance using random matrix theory.
	...
	% The scattered/reflected signals contain no additional information, but adds constructively (or destructively) with the direct component to enhance desired link (or suppress interference).
	% The scattered signals contain no additional information, but adds constructively or destructively with the direct component to enhance desired signal or suppress interference, improving spectrum and energy efficiency.
	...
	% Specifically, different IRS reflec- tion coefficients are set for different time slots in each channel coherence block to enable more flexible reflection design with generally less number of users selected to be served at each time slot, thus leading to a higher passive beamforming gain
	% Extensive research has been devoted to optimizing \gls{ris} phase shifts to improve communication, sensing, and power performances.
	...
	% To create artificial diversity and enable flexible resource allocation, the concept of dynamic passive beamforming was first proposed for \gls{ofdm} systems \cite{Yang2020},
	% The concept of dynamic passive beamforming that chooses \gls{ris} reflection coefficients independently over different time slots within each channel block was first proposed for \gls{ofdm} systems \cite{Yang2020}, then extended to multi-user \gls{wpt} \cite{Qiu2022} and \gls{wpcn} \cite{Wu2021d,Hua2022a} with dedicated downlink power and uplink information phases.
	% The concept of dynamic passive beamforming, namely choosing \gls{ris} reflection coefficients independently over different time slots within each channel block, was first proposed for \gls{ofdm} systems \cite{Yang2020}, then extended to multi-user \gls{wpt} \cite{Qiu2022} and \gls{wpcn} \cite{Wu2021d,Hua2022a} with dedicated downlink power and uplink information phases.
	% The concept of dynamic \gls{ris}, namely choosing independent reflection coefficients for different time slots within channel block, was first proposed to assist \gls{ofdm} systems \cite{Yang2020}, then extended for the downlink power and uplink information phases of \gls{wpcn} \cite{Wu2021,Wu2021d,Hua2022a}.
	...
	% Interestingly, \cite{Karasik2020} reported from information-theoretic perspective that if
	...
	% From an information-theoretic perspective, \cite{Karasik2020} reported using \gls{ris} as a pure auxiliary device to maximize the \gls{snr} at the receiver is generally rate-suboptimal for finite input constellations.
	% The capacity-achieving transmission scheme is to joint
	...
	% In particular, \emph{overlay modulation} decouples the overall reflection pattern as the product of an information-bearing symbol and an auxiliary coefficient matrix.
	% In particular, \emph{overlay modulation} superposes information-bearing symbols over a common auxiliary matrix \cite{Xu2020b,Zhang2021d,Hu2021b,Hua2022}, \emph{spatial modulation} designs the reflection coefficients to maximize the \gls{snr} of different
	% In particular, \emph{overlay modulation} superposes information-bearing symbols over a common auxiliary matrix \cite{Xu2020b,Zhang2021d,Hu2021b,Hua2022}, \emph{spatial modulation} varies \gls{ris} reflection coefficients to maximize the \gls{snr} at different receive antennas \cite{Basar2020,Ma2020a,Yuan2021}, \emph{index modulation} divided the \gls{ris} into reflection and information elements, and evaluated the error performance of non-coherent backscatter detection.
	% In particular, \emph{overlay modulation} superposes information-bearing symbols over a common auxiliary matrix \cite{Xu2020b,Zhang2021d,Hu2021b,Hua2022}, \emph{spatial modulation} varies \gls{ris} reflection coefficients to maximize the \gls{snr} at different receive antennas \cite{Basar2020,Ma2020a,Yuan2021}, \emph{index modulation} divided the \gls{ris} into reflection elements for passive beamforming and information elements
	% In particular, \emph{overlay modulation} superposes information-bearing symbols over a common auxiliary matrix \cite{Xu2020b,Zhang2021d,Hu2021b,Hua2022}, \emph{spatial modulation} switches between reflection coefficient sets that maximize \gls{snr} at different receive antennas \cite{Basar2020,Ma2020a,Yuan2021}, \emph{index modulation} divides \gls{ris} into reflection elements for passive beamforming and information elements for on-off keying modulation \cite{Hu2021a}, and \emph{symbol level precoding} designs the reflection coefficients to
	% In particular, \emph{overlay modulation} superposes information-bearing symbols over a common auxiliary matrix \cite{Xu2020b,Zhang2021d,Hu2021b,Hua2022}, \emph{spatial modulation} switches between reflection coefficient sets that maximize \gls{snr} at different receive antennas \cite{Basar2020,Ma2020a,Yuan2021}, \emph{index modulation} divides \gls{ris} into reflection elements for passive beamforming and information elements for on-off modulation \cite{Hu2021a}, and \emph{symbol level precoding} creates a mapping from reflection coefficients to backscatter symbol \cite{Liu2019d,Bereyhi2020} where .
	% In particular, \emph{overlay modulation} superposes information-bearing symbols over a common auxiliary matrix \cite{Xu2020b,Zhang2021d,Hu2021b,Hua2022}, \emph{spatial modulation} switches between reflection coefficient sets that maximize \gls{snr} at different receive antennas \cite{Basar2020,Ma2020a,Yuan2021}, \emph{index modulation} divides \gls{ris} into reflection elements for passive beamforming and information elements for on-off modulation \cite{Hu2021a}, and \emph{symbol level precoding} maps reflection coefficients to backscatter symbols and optimizes to improve detection performance.
	% In particular, \emph{overlay modulation} superposes information-bearing symbols over a common auxiliary matrix \cite{Xu2020b,Zhang2021d,Hu2021b,Hua2022}, \emph{spatial modulation} switches between reflection coefficient sets that maximize \gls{snr} at different receive antennas \cite{Basar2020,Ma2020a,Yuan2021}, \emph{index modulation} divides \gls{ris} into reflection elements for passive beamforming and information elements for on-off modulation \cite{Hu2021a}, and \emph{symbol level precoding} maps reflection coefficients to backscatter symbols and optimizes for detection performance.
	% In particular, \emph{symbol level precoding} maps optimized \gls{ris} coefficient sets to backscatter symbols to improve detection performance,
	% In particular, \emph{symbol level precoding} maps \gls{ris} coefficient sets to backscatter symbols and optimizes for detection performance,
	% In particular, \emph{symbol level precoding} maps optimized \gls{ris} coefficient sets to backscatter symbols \cite{Liu2019d,Bereyhi2020}, \emph{overlay modulation} superposes information-bearing symbols over a common auxiliary matrix \cite{Xu2020b,Zhang2021d,Hu2021b,Hua2022}, \emph{spatial modulation} switches between reflection coefficient sets that maximize \gls{snr} at different receive antennas \cite{Basar2020,Ma2020a,Yuan2021}, and \emph{index modulation} divides \gls{ris} into reflection elements for passive beamforming and information elements for on-off modulation \cite{Hu2021a}.
	...
	% \cite{Liu2019d,Bereyhi2020}


	% How to combine backscatter modulation and passive beamforming
	...
	When joint transmitter-\gls{ris} encoding is possible, \cite{Karasik2020} proved using \gls{ris} only for passive beamforming is generally suboptimal in terms of achievable rate for finite input constellations.
