...
\begin{section}{Simulation Results}
	...
	\begin{subsection}{Comparison of Scattering Applications}
		...
		In terms of maximum primary achievable rate, RIScatter coincides with \gls{ris} and outperforms the others using deterministic reflection pattern (no need for backscatter detection) when the primary link is prioritized.
		...
		Importantly, such a semi-coherent backscatter detection is feasible for arbitrary $N$, and using a fast-switching node with small $N$ can increase the backscatter throughput per unit time and preserve the dynamic passive beamforming gain.
		% It only requires one energy comparison, re-encoding, and precoding operations to decode each backscatter symbol (tuple).
	\end{subsection}
