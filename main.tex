...
\begin{section}{Introduction}
	...
	% \IEEEPARstart{I}{nternet-of-Everything} is envisioned to provide high throughput, ultralow power, ubiquitous distribution, and connected intelligence for future wireless networks.
	% \IEEEPARstart{F}{uture} wireless network is envisioned to provide high throughput, uniform coverage, massive connectivity and heterogeneous control for trillions of portable devices with ultralow power consumption.
	...
	% \gls{bc} is an emerging technique
	% that enables low-power communication by separating the
	% As an emerging low-power communication technique, \gls{bc} separates the power-hungry \gls{rf} components (i.e., synthesizer and amplifier) as dedicated carrier emitter, and employs passive energy harvester and impedance modulator to operate over incident and scattered signals.
	% As an emerging low-power communication technique, \gls{bc} separates the power-hungry \gls{rf} components (i.e., synthesizer and amplifier) as dedicated carrier emitter, and equips the backscatter node with energy harvester and load-switching modulator to operate over incident and scattered signals, respectively.
	% As an emerging low-power communication protocol, \gls{bc} divides transmitter into a dedicated carrier emitter with power-hungry \gls{rf} elements (e.g., synthesizer and amplifier) and an information-bearing node with passive components (e.g., harvester and modulator).
	...
	% \gls{bc} is an emerging low-power communication protocol that divides transmitter into a \gls{rf} signal emitter with power-hungry elements (e.g., synthesizer and amplifier) and an information-bearing scatter node with passive components (e.g., harvester and modulator).
	% The node harvests energy from emitted \gls{rf} wave and embeds its information over scattered signal in a sustainable and controllable manner, bringing more possibilities to \gls{ioe} and \gls{m2m} networks.
	% The node harvests energy from emitted \gls{rf} wave and embeds its information over scattered signal in a sustainable and controllable manner, which is widely applied in \gls{ioe} and \gls{m2m} networks.
	...
	% \gls{rfid}, as a main application of \gls{bc}, has experienced great success
	...
	% It requires a dedicated sinusoidal source
	...
	% Specifically, a dedicated emitter generates sinusoidal carrier at reserved frequency, and the node reports its identifier to a nearby \gls{rfid} reader.


	% Recently, RFID has experienced a tremendous growth due to developments in integrated circuits and radios, and due to increased interest from retail industry and government [4].
	...
	% Since the node harvests energy from emitted \gls{rf} wave and embeds its information over scattered signal, such that
	% The rapid growth of wireless devices and the steady decrease of electronic power consumption

	% account for the
	% Although \gls{bc} experienced

	% the power-hungry \gls{rf} elements (e.g., synthesizer and amplifier) as dedicated carrier emitter, and leaves the passive components (e.g., harvester and modulator) at the backscatter node.
	% The node harvests energy from emitted \gls{rf} wave and embeds its information over scattered signal.


	% alter scattered electromagnetic field and transmit without onboard \gls{rf} components (e.g., carrier syntheizer and power amplifier).
	% Passive backscatter nodes harvest energy from surrounding \gls{rf} waves


	% The evolution in the field of wireless communications requires not only futuristic technologies but also, a change of mindset about how different technologies interact with each other like a well-oiled machine. In this backdrop, the deployment of massive Internet of things (IoT) networks poses significant challenges. While there have been numerous technologies proposed to improve the performance of such low-powered IoT networks, the current state of the art is still not mature enough to deploy commercial and large-scale networks. Rapid energy consumption along with miniature size of energy reservoirs naturally becomes an obstacle as these IoT devices are likely to be untethered when operating. Fortunately, recent research efforts have shown that radio backscatter communication can effectively offer near-zero-power (less than 1mW) connectivity to small devices. In this regard, this book explores the performance evaluation and optimization aspects of backscatter devices in outdoor and indoor environments, where it is not possible to set up a dedicated nearby transmitter. The book also sheds light on different cooperation techniques for backscatter communications. These techniques help analyze the reliability-energy trade-off for backscatter devices. Also, there are in-depth dis- cussions and recent studies on a variety of emerging wireless topics such as physical-layer security, non-orthogonal multiple access (NOMA), age of informa- tion, and multi-tone carrier techniques.

	% \IEEEPARstart{S}{catterers} can harvest energy from, modulate information over, and influence propagation of surrounding radio waves.
