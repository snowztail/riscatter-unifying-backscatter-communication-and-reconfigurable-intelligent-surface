...
\begin{abstract}
	...
	\gls{ris} adapts scattering antennas or programmable metamaterial to control propagation environment and boost signal strength in desired direction.
	...
	In this paper, we introduce UniScatter as a novel paradigm that bridges and generalizes those three concepts to support the high throughput, ultralow power, and ubiquitous connectivity of future wireless networks.
	...
	% To accommodate signal characteristics, we propose a practical receiver that semi-coherently decodes all nodes from accumulated energy, re-encodes to determine the equivalent channel, and decodes the primary link.
	% To accommodate signal characteristics, we propose a practical receiver that first semi-coherently decodes all nodes by energy detection, then re-encodes and determines the equivalent channel for primary decoding.
	...
	Simulation results demonstrate UniScatter can unify \gls{bc}, \gls{ris} and \gls{sr} to flexibly control the transmit-assist tradeoff via smart input distribution design.
