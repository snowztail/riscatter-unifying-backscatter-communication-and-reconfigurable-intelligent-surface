...
\begin{section}{Introduction}
	...
	% \emph{Second,} we propose a practical UniScatter receiver that semi-coherently decodes UniScatter nodes from the received energy during each backscatter symbol block, re-encodes and recovers their reflection patterns, combines with relevant \gls{csi} for equivalent primary channel as if dynamic passive beamforming, then coherently decodes the primary link under the enhanced multipath.
	\emph{Second,} we propose a practical UniScatter receiver that semi-coherently decodes UniScatter nodes from the received energy during each backscatter symbol block, re-encodes for their reflection patterns, combines with relevant \gls{csi} as if dynamic passive beamforming, then coherently decodes the primary link under the enhanced multipath.
...
\begin{section}{UniScatter}
	Using shared spectrum, energy and infrastructures, UniScatter is a novel passive communication/assistance protocol that generalizes \gls{bc}, \gls{sr} and \gls{ris} from the perspective of distribution.
	In particular, passive UniScatter nodes ride over an active legacy transmission in a flexible and mutualistic manner, while practical UniScatter receiver cooperatively decodes both links under the benefits of backscatter modulation and passive beamforming.
	% TODO compare BC, SR and IRS
	\begin{subsection}{UniScatter Nodes}
		Based on link priority and \gls{csi}, UniScatter nodes adapt the \emph{state input probability distribution} of antenna-based scatterers to unify and generalize backscatter modulation and passive beamforming.
		...
		\begin{figure}[!t]
			...
			\caption{
				...
				Within channel coherence time, UniScatter nodes semi-randomly select reflection state for each backscatter symbol block with the guidance of input probability distribution.
			}
		...
		That is, it semi-randomly select the reflection state during each backscatter symbol block with the guidance of input probability $P(\Gamma_m)$ for state $m$.
		Such an adaptive backscatter channel coding coincides with \gls{ris} when primary link is absolutely prioritized, and outperforms the uncoded equiprobable transmission of \gls{bc}/\gls{sr} when backscatter link is absolutely prioritized.
		UniScatter nodes can be implemented over readily available passive backscatter devices with energy harvester and integrated receiver \cite{Kim2021a}, while the relevant \gls{csi} can be efficiently and accurately acquired using the state-of-the-art parallel approach \cite{Jin2021a}.
		\begin{figure*}[!t]
			\centering
		...
		\end{figure*}
		The block diagram, equivalent circuit, and scatter model of a UniScatter node are illustrated in Fig. \ref{fi:uniscatter_node}.
		A proportion of the impinging wave emitted by primary transmitter is harvested for the operation of UniScatter node, while reradiated component includes a (potentially) random phase shift due to backscatter message.
		% Next, we will discuss the input design in a UniScatter network.
	\end{subsection}
	...
	\begin{subsection}{UniScatter Receiver}
		...
		Besides, the backscatter symbol period is typically longer than primary due to the load switching constraint.
		% ? In such case, the advantages of power-domain \gls{noma} with \gls{sic} receiver becomes questionable.
		% Those facts imply if the backscatter messages can be decoded first, one can recover the reflection patterns of all UniScatters and model them within the equivalent primary channel as if dynamic passive beamforming.
		Both facts imply backscatter detection under primary uncertainty is also part of equivalent primary channel training.
		...
		\end{figure}
		As illustrated in Fig. \ref{fi:energy_distribution}, the total received energy during each backscatter symbol block is a random variable that follows different distributions conditioned on different input state hypotheses.
	...
	\begin{subsection}{System Model}
		...
		\end{figure}
		In this subsection, we consider a single-user multi-node \gls{miso} UniScatter network as shown in Fig. \ref{fi:uniscatter_network}.
...
\begin{section}{Input Distribution, Active Beamforming, and Decision Threshold Design}
	To characterize the achievable primary-total-backscatter rate region of the proposed UniScatter network, we aim to maximize the weighted sum mutual information with respect to node input distributions $\{\boldsymbol{p}_k\}_{k \in \mathcal{K}}$, active beamforming vector $\boldsymbol{w}$, and decision threshold vector $\boldsymbol{t}$ as
	\begin{maxi!}
