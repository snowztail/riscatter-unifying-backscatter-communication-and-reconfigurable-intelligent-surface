	...
	\begin{section}{Input Distribution, Decision Region, and Precoder Design}
		...
		\begin{subsection}{Input probability distribution}
			...
			\begin{proposition}
				...
				\begin{subequations}
					\begin{alignat}{2}
						I_k(\bar{c}_{m_k};\boldsymbol{y},z) & = I(c_{\mathcal{K}};\boldsymbol{y},z), \quad && P_k^{\star}(\bar{c}_{m_k}) > 0,\label{co:probable_states}\\
						I_k(\bar{c}_{m_k};\boldsymbol{y},z) & \le I(c_{\mathcal{K}};\boldsymbol{y},z), \quad && P_k^{\star}(\bar{c}_{m_k}) = 0.\label{co:dropped_states}
					\end{alignat}
				\end{subequations}
			...
			\begin{proof}
				...
				\begin{subequations}
					\begin{alignat}{2}
						I_k(\bar{c}_{m_k};\boldsymbol{y},z) & = \mu_k^{\star} + (1 - \rho), \quad && P_k^{\star}(\bar{c}_{m_k}) > 0,\\
						I_k(\bar{c}_{m_k};\boldsymbol{y},z) & \le \mu_k^{\star} + (1 - \rho), \quad && P_k^{\star}(\bar{c}_{m_k}) = 0,
					\end{alignat}
				\end{subequations}
				which implies
				\begin{equation}
			...
			\begin{theorem}
				The input probability distribution that achieves the weighted sum primary-backscatter capacity at state $i_k \in \mathcal{M}$ of tag $q \in \mathcal{K}$ is given by the limit of the following sequence\footnote{Note that the probability distribution of tag $q$ is based on the updated probability distribution of tags $1,\ldots,k-1$.}
				\begin{equation}
			...
			\end{theorem}
			\begin{proof}
				We first prove that for tag $q \in \mathcal{K}$, updating the probability of state $i_q \in \mathcal{M}$ by sequence~\eqref{eq:optimal_distribution} does not decrease mutual information. For any input combination $\bar{c}_{m_{\mathcal{K}}}$, let $P_{\mathcal{K}}(\bar{c}_{m_{\mathcal{K}}}) = \prod_{k \in \mathcal{K}} P_k(\bar{c}_{m_k})$ and $P_{\mathcal{K}}'(\bar{c}_{m_{\mathcal{K}}}) = P_q'(\bar{c}_{m_q}) \prod_{k \in \mathcal{K} \setminus \{q\}} P_k(\bar{c}_{m_k})$ be two probability distributions with potentially different marginal for tag $q$, and denote the corresponding mutual information and received energy distribution as $I(\bar{c}_{\mathcal{K}};\boldsymbol{y},z)$, $I'(\bar{c}_{\mathcal{K}};\boldsymbol{y},z)$ and $P(\bar{z}_j)$, $P'(\bar{z}_j)$, respectively. Also, define an intermediate function $J \left( P_{\mathcal{K}}(\bar{c}_{m_{\mathcal{K}}}),P_{\mathcal{K}}'(\bar{c}_{m_{\mathcal{K}}}) \right)$ by \eqref{eq:intermediate_information_function}, and apparently $I(\bar{c}_{\mathcal{K}};\boldsymbol{y},z) = J \left( P_{\mathcal{K}}(\bar{c}_{m_{\mathcal{K}}}),P_{\mathcal{K}}(\bar{c}_{m_{\mathcal{K}}}) \right)$.
				\begin{figure*}[!b]
					\hrule
					\begin{align}
					...
					\end{align}
				\end{figure*}
				For a fixed $P_{\mathcal{K}}'(\bar{c}_{m_{\mathcal{K}}})$, $J \left( P_{\mathcal{K}}(\bar{c}_{m_{\mathcal{K}}}),P_{\mathcal{K}}'(\bar{c}_{m_{\mathcal{K}}}) \right)$ is a concave function of $P_q(\bar{c}_{m_q})$ and can be maximized at $\nabla_{P_q^{\star}(\bar{c}_{m_q})} J \left( P_{\mathcal{K}}(\bar{c}_{m_{\mathcal{K}}}),P_{\mathcal{K}}'(\bar{c}_{m_{\mathcal{K}}}) \right) = 0$, which reduces to
				\begin{equation}
				...
				\begin{subequations}
					\begin{align}
						...
						& = \frac{P_q'(\bar{c}_{m_q}) \exp \left( \frac{\rho}{1 - \rho} I_q(\bar{c}_{m_q};\boldsymbol{y},z) \right)}{\sum_{m_q} P_q'(\bar{c}_{m_q}) \exp \left( \frac{\rho}{1 - \rho} I_q(\bar{c}_{m_q};\boldsymbol{y},z) \right)}.
						\label{eq:optimal_relative_distribution}
					\end{align}
				\end{subequations}

				Although optimality is not guaranteed for state $i_q$, we notice that $P_q(\bar{c}_{i_q}) = 1 - \sum_{m_q \ne i_q} P_q^{\star}(\bar{c}_{m_q})$ has exactly the same form as \eqref{eq:optimal_relative_distribution}. It suggests that the solution of choosing $i_q$ as reference would coincide with that of choosing any $m_q \ne i_q$ as reference, while optimality is guaranteed for $i_q$ in the latter case. Therefore, for a fixed $P_{\mathcal{K}}'(\bar{c}_{m_{\mathcal{K}}})$, $\forall i_q \in \mathcal{M}$ and $\forall q \in \mathcal{K}$, \eqref{eq:optimal_relative_distribution} ensures
				\begin{equation}
				...
				On the other hand, choosing $P_q(\bar{c}_{m_q})$ by \eqref{eq:optimal_relative_distribution} also satisfies
				\begin{subequations}
					...
					\begin{align}
						...
						& = (1 - \rho)
						\sum_{m_{\mathcal{K}}} P_{\mathcal{K}}(\bar{c}_{m_{\mathcal{K}}}) \sum_j P(\bar{z}_j \mid \bar{c}_{m_{\mathcal{K}}}) \log \frac{P'(\bar{z}_j) P_q(\bar{c}_{m_q})}{P(\bar{z}_j) P_q'(\bar{c}_{m_q})}\\
						& = (1 - \rho) \sum_{m_q} \frac{P_q'(\bar{c}_{m_q}) u_q(\bar{c}_{m_q})}{\sum_{i_q} P_q'(\bar{c}_{i_q}) u_q(\bar{c}_{i_q})} \sum_j P(\bar{z}_j \mid \bar{c}_{m_q})\nonumber\\
						& \quad \times \log \frac{\sum_{m_q'} P_q'(\bar{c}_{m_q'}) P(\bar{z}_j \mid \bar{c}_{m_q'}) u_q(\bar{c}_{m_q})}{\sum_{i_q'} P_q'(\bar{c}_{i_q'}) P(\bar{z}_j \mid \bar{c}_{i_q'}) u_q(\bar{c}_{i_q'})}\\
						& \ge (1 - \rho) \sum_{m_q} \frac{P_q'(\bar{c}_{m_q}) u_q(\bar{c}_{m_q})}{\sum_{i_q} P_q'(\bar{c}_{i_q}) u_q(\bar{c}_{i_q})} \sum_j P(\bar{z}_j \mid \bar{c}_{m_q})\nonumber\\
						& \quad \times \left( 1 - \frac{\sum_{i_q'} P_q'(\bar{c}_{i_q'}) P(\bar{z}_j \mid \bar{c}_{i_q'}) u_q(\bar{c}_{i_q'})}{\sum_{m_q'} P_q'(\bar{c}_{m_q'}) P(\bar{z}_j \mid \bar{c}_{m_q'}) u_q(\bar{c}_{m_q})} \right)\\
						& = 0,
					\end{align}
				\end{subequations}
				where $u_q(\bar{c}_{m_q}) \triangleq \exp \left( \frac{\rho}{1 - \rho} I_q(\bar{c}_{m_q};\boldsymbol{y},z) \right)$ and the equality holds if and only if \eqref{eq:optimal_relative_distribution} converges. Combining \eqref{eq:information_difference_lower} and \eqref{eq:information_difference_upper}, we conclude that update the input probability distribution of tag $q \in \mathcal{K}$ by \eqref{eq:optimal_distribution} does not decrease mutual information.
