...
\begin{section}{Introduction}
	...
	However, it assumed equal symbol duration and perfect synchronization for the coexisting links.
	...
	In contrast, \cite{Vardakis2023} exploits commodity \gls{rfid} tags, powered and controlled by a software-defined radio reader, to perform passive beamforming (but no backscatter modulation) towards a legacy user at a different frequency.
	...
	For binary-input \gls{ambc}, the achievable rate and optimal input distribution are investigated in \cite{Qian2019b}, but the authors did not consider its impact on the primary link.
	...
	This probability distribution is carefully designed to incorporate the backscatter information, \gls{csi}, and \gls{qos}\footnote{\gls{qos} refers to the relative priority of the primary link.}.
...
\begin{section}{RIScatter}
	\begin{subsection}{Principles}
		...
		Its impact is usually modelled as part of environment multipath \cite{Thomas2012,Liang2020}, or simply a DC offset in the baseband signal when the impinging signal is a \gls{cw} \cite{Boyer2014}.
		...
		RIScatter nodes can be implemented, for example, by adding an integrated receiver\footnote{The aim is to coordinate the node with the active source and to acquire the optimized input distribution, instead of decoding the primary information dedicated for the user. The node receiver can be implemented using very simple circuits or even integrated with the rectifier \cite{Kim2021a} to reduce cost and complexity.} \cite{Kim2021a} and adaptive encoder \cite{He2020e} to off-the-shelf passive \gls{rfid} tags.
...
\begin{section}{Simulation Results}
	...
	\begin{subsection}{Comparison of Scattering Applications}
		...
		\begin{itemize}
			...
			The primary achievable rate is approximately\footnote{The scattered component is treated as interference with average power $\mathbb{E}\bigl\{\sum_{k} \alpha_k \boldsymbol{h}_{\text{C},k}^\mathsf{H} x_k \boldsymbol{w}s[n]\bigr\} = \sum_{k} \lvert \alpha_k \boldsymbol{h}_{\text{C},k}^\mathsf{H} \boldsymbol{w} \rvert^2$ \cite{Long2020a}.}
			\begin{equation}
