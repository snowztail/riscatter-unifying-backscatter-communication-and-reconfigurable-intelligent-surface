...
\begin{section}{Introduction}
	...
	\IEEEPARstart{F}{uture} wireless network is envisioned to provide high throughput, ultralow power, ubiquitous deployment, and connected intelligence for trillions of portable devices.
	% \gls{bc} is an emerging technique that allows low-power devices to transmit without power-hungry \gls{rf} components.
	% An emerging technique for low-power communication is \gls{bc}, which allows backscatter node to embed its own message over
	% An emerging technique for low-power communication is \gls{bc}, which allows information node to embed its own message over
	% transmit without active \gls{rf} components
	% \gls{bc} is an emerging technique that allows information nodes to alter scattered electromagnetic field and transmit without onboard \gls{rf} components (e.g., carrier syntheizer and power amplifier).
	% \gls{bc} is an emerging technique that allows information nodes to operate over surrounding carrier wave and transmit without onboard \gls{rf} components.
	\gls{bc} is an emerging technique that allows backscatter nodes to operate over dedicated carrier signal without onboard \gls{rf} components (e.g., carrier syntheizer and power amplifier).
	The low-power nodes harvest energy from incident wave and modulate information over scattered signal.
