...
\begin{section}{RIScatter}
	\begin{subsection}{Concepts}
		As a generalization of \gls{bc}, \gls{sr} and \gls{ris}, RIScatter is a passive scattering protocol that coexists with an active primary link in a flexible and mutualistic manner.
		...
		\begin{remark}
			...
			\footnote{
				...
				For scatter-based multiple access, \gls{sic} not only fails to utilize the double modulation, but also requires $N$ re-encoding, precoding and subtraction with a time-domain \gls{mrc} during each backscatter block.
			}
		...
		It enables backscatter modulation and dynamic passive beamforming using only one energy level comparison, re-encoding and precoding for each backscatter block (instead of primary block as \gls{sic}), and is suitable for arbitrary primary and backscatter symbol periods.
	\end{subsection}
