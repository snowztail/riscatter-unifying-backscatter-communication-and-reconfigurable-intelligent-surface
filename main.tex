...
\begin{section}{UniScatter}
	...
	\begin{subsection}{System Model}
		...
		\begin{equation}
			\mathcal{R}_{l} \triangleq [t_{l-1},t_l), \quad t_{l-1} \le t_l.
		\end{equation}
...
\begin{section}{Rate-Region Characterization}
	...
	\begin{maxi!}
		...
		\addConstraint{\boldsymbol{t}}{\ge \boldsymbol{0}.}{\label{co:decision_threshold}}
	\end{maxi!}
	...
	\begin{subsection}{Decision Threshold}
		...
		Fortunately, we can further simplify problem \eqref{op:decision_threshold} as a point-to-point rate-optimal quantization design for a discrete-input continuous-output memoryless channel, thanks to Remark \ref{re:backscatter_decision} and \ref{re:augmented_source}.
		...
		\begin{figure}[!t]
			...
			\caption{
				The decision thresholds are selected from fine-grained discrete energy levels instead of continuous space, and each decision region consists of at least one neighbor energy bins.
			}
			\label{fi:discrete_outputs}
		\end{figure}
		As shown in Fig. \ref{fi:discrete_outputs}, by introducing the extra analog-to-digital conversion step, we can obtain decision regions by grouping adjacent high-resolution energy bins.
		Thus, problem \eqref{op:decision_threshold} is recast as
		\begin{maxi}
			{\scriptstyle{\boldsymbol{t} \in \mathcal{T}^{L+1}}}{I_{\mathrm{B}}(x_{\mathcal{K}};\hat{x}_{\mathcal{K}}),}{\label{op:decision_threshold_recast}}{}
		\end{maxi}
		which can be solved by existing rate-optimal \gls{dmc} quantizer designs.
		For global optimal solutions, \cite{He2021} proposed a \gls{dp} method accelerated by the \gls{smawk} algorithm with computational complexity $\mathcal{O}\bigl(L^2(\mathrm{card}(\mathcal{T})-L)\bigr)$, and \cite{Nguyen2020a} presented a traverse-bisection algorithm using the optimality condition of three neighbor thresholds with complexity $\mathcal{O}\bigl(\mathrm{card}(\mathcal{T})L\log(\mathrm{card}(\mathcal{T})L)\bigr)$.
		In Section \ref{se:simulation_results}, both schemes will be compared with the \gls{ml} thresholding scheme \cite{Qian2019}
		\begin{equation}
