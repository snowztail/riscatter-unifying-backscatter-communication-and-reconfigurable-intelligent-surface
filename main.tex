...
\begin{section}{Introduction}
	...
	% In contrast, sequential decoding is subject to uncertainty/interference at the first layer \cite{Ding2020,Long2020a,Zhou2019a,Wu2021a,Xu2021a,Yang2021a,Yang2018,Han2021,Zhang2022,Xu2020b,Hua2022}.
	% Due to the physical constraint at the load-switching modulator, backscatter symbol duration can be much longer than primary.
	...
	% \cite{Long2020a} indicated if backscatter symbol is sufficiently long, its randomness can be modelled as channel uncertainty, and primary
	% \cite{Long2020a} pointed out if backscatter symbol is sufficiently long, its randomness can be modelled as channel uncertainty, and
	% \cite{Long2020a} pointed out when decoding the primary link first, the randomness of backscatter modulation can be modelled as channel uncertainty.
	...
	% However, the advantage of this particular scheme is questionable because 1) it does not fully exploit backscatter pattern and signal characteristics; 2) it requires non-coherent encoding at the primary transmitter, and re-encoding, precoding, and subtraction at the receiver; 3) the primary and backscatter symbols are mixed by multiplication instead of superposition.
	However, the advantage of this particular scheme is questionable because 1) it does not fully exploit backscatter pattern and signal characteristics; 2) non-coherent primary encoding is required at the transmitter while re-encoding, precoding, and subtraction are required at the receiver; 3) the primary and backscatter symbols are mixed by multiplication instead of superposition.

	% This receiving scheme, as would be discussed later, may not fully

	% Besides, \cite{Long2020a} concluded if the backscatter symbol period is sufficiently long, then the non-coherent primary rate would approach its coherent counterpart.
