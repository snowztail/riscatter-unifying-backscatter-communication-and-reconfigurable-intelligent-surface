...
\begin{document}
	...
	\begin{section}{Backscatter Detection and Achievable Rates}
		...
		\begin{subsection}{Sum backscatter rate}
			...
			% 	\label{eq:backscatter_information_function}
			% \end{align}
			the backscatter marginal information function of tag $k$ associated with $\bar{c}_{m_{k}}$ is defined as
			\begin{equation}
				I_{\mathrm{B},k}(\bar{c}_{m_{k}} ; z) \triangleq \sum_{m_{\mathcal{K} \setminus \{k\}}} \prod_{k' \in \mathcal{K} \setminus \{k\}} P_{k'}(\bar{c}_{m_{k'}}) I_{\mathrm{B}}(\bar{c}_{m_{\mathcal{K}}} ; z),
				\label{eq:backscatter_marginal_information_function}
			\end{equation}
			% \begin{align}
			% 	I_{\mathrm{B},k}(\bar{c}_{m_{k}} ; z)
			...
			% 	\label{eq:backscatter_marginal_information_function}
			% \end{equation}
			and the backscatter mutual information can be expressed as\footnote{Please be aware that $c_{\mathcal{K}}$, $c_{\mathcal{S}}$, $z$ are random variables, while $\bar{c}_{m_{\mathcal{K}}}$ and $\bar{c}_i$, $\bar{c}_{m_{\mathcal{S}}}$, $\bar{z}_j$ represent the corresponding instances.}
			\begin{align}
				I_{\mathrm{B}}(c_{\mathcal{K}} ; z)
				& = \mathbb{E}_{c_{\mathcal{K}}} \left[I_{\mathrm{B}}(\bar{c}_{m_{\mathcal{K}}} ; z)\right] = \mathbb{E}_{c_{k}} \left[I_{\mathrm{B},k}(\bar{c}_{m_{k}} ; z)\right]\\
		...
		\begin{subsection}{Ergodic primary rate}
			...
			% 	\label{eq:primary_marginal_information_function}
			% \end{equation}
			and the ergodic primary rate can be expressed as \footnote{The unit of \eqref{eq:backscatter_information_function}, \eqref{eq:backscatter_marginal_information_function} and \eqref{eq:backscatter_sum_rate} are \gls{bpcu}, while the unit of \eqref{eq:primary_information_function}, \eqref{eq:primary_marginal_information_function} and \eqref{eq:primary_ergodic_rate} are \gls{bpsphz}.}
			\begin{align}
				I_{\mathrm{P}}(c_{\mathcal{K}} ; \boldsymbol{y})
				& = \mathbb{E}_{c_{\mathcal{K}}} \left[I_{\mathrm{P}}(\bar{c}_{m_{\mathcal{K}}} ; \boldsymbol{y})\right] = \mathbb{E}_{c_{k}} \left[I_{\mathrm{P},k}(\bar{c}_{m_{k}} ; \boldsymbol{y})\right]\\
	...
	\begin{section}{Input Distribution, Decision Region, and Precoder Design}
		\begin{subsection}{Primary-backscatter rate region}
			First, we define the weighted sum marginal information of tag $k$ associated with symbol $\bar{c}_{m_{k}}$ and the weighted sum primary-backscatter rate respectively as
			\begin{align}
				I_{q}(\bar{c}_{m_{k}} ; \boldsymbol{y},z)
				& \triangleq \rho I_{\mathrm{P},q}(\bar{c}_{m_{k}} ; \boldsymbol{y}) + (1 - \rho) I_{\mathrm{B},q}(\bar{c}_{m_{k}} ; z),\\
				I(c_{\mathcal{K}} ; \boldsymbol{y}, z)
				& \triangleq \rho I_{\mathrm{P}}(c_{\mathcal{K}} ; \boldsymbol{y}) + (1 - \rho) I_{\mathrm{B}}(c_{\mathcal{K}} ; z)
			\end{align}
			...
			As problem \eqref{op:rate_region} is not jointly convex over $\boldsymbol{P}$, $\boldsymbol{t}$ and $\boldsymbol{w}$, we propose a \gls{bcd} method that iteratively updates the input distribution, decision region, and transmit precoder, until convergence is achieved.
		\end{subsection}
		...
		\begin{subsection}{Input probability distribution}
			...
			\begin{subequations}
				...
				\end{equation}
			\end{subequations}
			and the partial derivative can be explicitly expressed as
			% \begin{equation}
			% 	\pdv{I}{P_q^{\star}(\bar{c}_{i_q})} = \rho I_{\mathrm{P},q}(\bar{c}_{i_q} ; \boldsymbol{y}) + (1 - \rho) \left( I_{\mathrm{B},q}(\bar{c}_{i_q} ; z) - 1 \right).
			% \end{equation}
			\begin{align}
				\pdv{I}{P_q^{\star}(\bar{c}_{i_q})}
				& = \rho I_{\mathrm{P},q}(\bar{c}_{i_q} ; \boldsymbol{y}) + (1 - \rho) \left( I_{\mathrm{B},q}(\bar{c}_{i_q} ; z) - 1 \right)\\
				& = I_{q}(\bar{c}_{i_q} ; \boldsymbol{y},z) - (1 - \rho).
			\end{align}
			which suggests $\mu_q^{\star}$ must satisfy
			\begin{subequations}
				\begin{equation}
					\mu_q^{\star} = I_{q}(\bar{c}_{i_q} ; \boldsymbol{y},z) - (1 - \rho), \quad P_q^{\star}(\bar{c}_{i_q}) > 0,
				\end{equation}
				\begin{equation}
					\mu_q^{\star} \ge I_{q}(\bar{c}_{i_q} ; \boldsymbol{y},z) - (1 - \rho), \quad P_q^{\star}(\bar{c}_{i_q}) = 0.
				\end{equation}
			\end{subequations}
			...
			\begin{subequations}
				\begin{equation}
					I_{q}(\bar{c}_{i_q} ; \boldsymbol{y},z) = C_q, \quad P_q^{\star}(\bar{c}_{i_q}) > 0,
				\end{equation}
				\begin{equation}
					I_{q}(\bar{c}_{i_q} ; \boldsymbol{y},z) \le C_q, \quad P_q^{\star}(\bar{c}_{i_q}) = 0,
				\end{equation}
			\end{subequations}
		\end{subsection}
