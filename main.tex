...
\begin{abstract}
	...
	Simulation results demonstrate RIScatter nodes can exploit the additional propagation paths to smoothly transition between backscatter modulation and passive beamforming via smart input distribution design, and RIScatter receiver significantly reduces the decoding complexity and effectively accommodates the double modulation and signal characteristics in scatter-based cooperative networks.
\end{abstract}
...
\begin{section}{Introduction}
	...
	\begin{subsection}{Related Works}
		...
		Instead, the capacity of \gls{ris}-aided channel is achieved by joint transmitter-\gls{ris} encoding, and multiplicative coding with \gls{sic} decoding (i.e., \gls{sic}-based \gls{sr}) can outperform pure passive beamforming at high \gls{snr}.
	...
	\begin{subsection}{Contributions}
		...
		\emph{First,} we propose RIScatter nodes that adapt the reflection state distribution of passive scatter devices based on \gls{csi} and link weights to flexibly transition between backscatter modulation and passive beamforming.
		Scattering sources of \gls{bc}/\gls{sr} and reflecting elements of \gls{ris} can be regarded as special cases where the input distribution becomes equiprobable and degenerate, respectively.
		On the contrary, adaptive RIScatter encoding can achieve higher backscatter rate than conventional line coding.
		The RIScatter nodes can be either co-located to enable joint distribution design for improved total backscatter rate, or dispersed to guarantee uniformly good primary and backscatter performances.
