...
\begin{section}{Introduction}
	...
	A \gls{bcd} algorithm is proposed where the \gls{kkt} input distribution is numerically evaluated by limit of sequences, the active beamforming is optimized by \gls{pga}, and the decision regions are refined by state-of-the-art sequential quantizer designs for \gls{dmtc}.
...
\begin{section}{RIScatter}
	...
	\begin{subsection}{System Model}
		...
		With sequential thresholding, the decision region of backscatter symbol tuple $l \in \mathcal{L} = \{1,\ldots,L\}$ is\footnote{$m_{\mathcal{K}}$ and $l$ are one-to-one mapped. Both notations are used interchangeably in the following context.}
		\begin{equation}
...
\begin{section}{Rate-Region Characterization}
	...
	To obtain the achievable primary-(total-)backscatter rate region, we consider the weighted sum mutual information maximization problem with independent encoding at all nodes\footnote{Joint encoding over multiple nodes can be viewed as the special case with an augmented source.\label{fn:joint_encoding}}
	\begin{maxi!}
...
\begin{section}{Simulation Results}
	...
	\begin{subsection}{Comparison of Scattering Applications}
		...
		This is because both RIScatter and \gls{ambc} employ semi-coherent energy detection, and the adaptive channel coding of RIScatter outperforms the equiprobable inputs of \gls{ambc}, especially at low backscatter \gls{snr}.
		When multiple antenna is available at the \gls{ap}, active beamforming can be optimized for RIScatter nodes and the backscatter rate gain over \gls{ambc} should be more prominent.
	\end{subsection}
	...
	\begin{subsection}{Rate Region by Different Schemes}
		...
		\begin{subsubsection}{Input Distribution}
			...
			It is observed that Cooperation achieves the outer bound of all schemes, but the backscatter rate gain comes at the cost of implementation and computation complexity.
			...
			Those gaps should be larger when $M$ or $K$ increases.
	...
	\begin{subsection}{Rate Region under Different Configurations}
		...
		\begin{subsubsection}{Number of Nodes}
			...
			On the other hand, when the primary link is prioritized, the RIScatter nodes boil down to \gls{ris} elements and enjoy a passive array gain $K^2$.
		\end{subsubsection}
		...
		\begin{subsubsection}{Spreading factor}
			...
			On the other hand, when $N$ is too small, energy detection is error-prone as the \gls{dmmac} \eqref{eq:dmmac} becomes unreliable.
