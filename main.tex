...
\begin{section}{Simulation Results}
	...
	\begin{subsection}{Comparison of Scattering Applications}
		...
		\begin{itemize}
			...
			\item \emph{\gls{sr}:} When $N$ is sufficiently large, the primary ergodic rate under semi-coherent detection can be approximated by \eqref{eq:primary_mutual_information} with uniform input distribution \cite{Long2020a}.
			When $s[n]$ is successfully decoded and the direct interference $\boldsymbol{h}_{\mathrm{D}}^H \boldsymbol{w} s[n]$ is perfectly cancelled, the intermediate signal used for backscatter detection is
			\begin{equation}
			...
			By the law of large numbers, as $N$ goes to infinity, the \gls{pdf} of accumulated intermediate energy approaches
			\begin{equation}
