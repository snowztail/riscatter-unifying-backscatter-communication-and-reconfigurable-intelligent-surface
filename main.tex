...
\begin{section}{Introduction}
	...
	\begin{subsection}{Related Works}
		...
		One special property of scatter-based multiple access is the backscatter signal strength is significantly weaker than primary due to the double fading effect.
	...
	\begin{subsection}{Contributions}
		...
		\emph{Second,} we propose a practical receiver that accommodates the double modulation and signal difference in scatter-based multiple access.
...
\begin{section}{RIScatter}
	\begin{subsection}{Concepts}
		% As a generalization of \gls{bc}, \gls{sr} and \gls{ris}, RIScatter is a passive scattering protocol that cooperatives with an active primary link in a flexible and mutualistic manner.
		% Specifically, RIScatter nodes utilizes \gls{csi}- and weight-based input distribution design to smoothly transition between backscatter modulation and passive beamforming, and a tailored cooperative receiver exploits signal properties (i.e., double modulation and symbol period difference) to effectively decode both primary and backscatter links.
		% RIScatter nodes can be implemented by adding an integrated receiver \cite{Kim2021a} and adaptive encoder \cite{He2020e} to off-the-shelf passive backscatter tags, and relevant \gls{csi} can be acquired by existing low-power estimation techniques.
		...
		% For co-located RIScatter nodes, channel estimation can be simplified by group-based \cite{Zheng2019} and hierarchical \cite{You2019} trainings originally proposed for \gls{ris}.
		...
		% Such an adaptive backscatter coding boils down to degenerate distribution of \gls{ris} when primary link is prioritized, and outperforms line coding with equiprobable inputs (e.g., FM0 for \gls{rfid}) when backscatter link is prioritized.
		% Besides, joint encoding over multiple co-located RIScatter nodes can further improve the total backscatter rate.

		As a generalization of \gls{bc}, \gls{sr} and \gls{ris}, RIScatter is a passive scattering protocol that cooperatives with an active primary link in a flexible and mutualistic manner.
		RIScatter nodes utilizes \gls{csi}- and weight-based input distribution design to smoothly transition between backscatter modulation and passive beamforming.
		...
		% , and a tailored cooperative receiver exploits signal properties (i.e., double modulation and symbol period difference) to effectively decode both primary and backscatter links.
		% RIScatter nodes can be implemented by adding an integrated receiver \cite{Kim2021a} and adaptive encoder \cite{He2020e} to off-the-shelf passive backscatter tags, and relevant \gls{csi} can be acquired by existing low-power estimation techniques.
		% For dispersed RIScatter nodes, the cascaded transmitter-node-receiver channels can be estimated by sequential \cite{Bharadia2015,Yang2015b,Guo2019g} or parallel approaches \cite{Jin2021a} originally proposed for \gls{bc}.
		% For co-located RIScatter nodes, channel estimation can be simplified by group-based \cite{Zheng2019} and hierarchical \cite{You2019} trainings originally proposed for \gls{ris}.
		...
		\begin{remark}
			...
			% Hence, \gls{sic}, which was originally proposed for superposition coding,
			It suggests backscatter detection under primary uncertainty can be viewed as part of primary channel training, and novel decoding strategies apart from \gls{sic}
			\footnote{
				Recall that \gls{sic} was proposed to achieve the capacity vertices for \gls{mac} with superposition coding.
				For scatter-based multiple access, \gls{sic} not only fails to exploit the double modulation effect, but also requires $N$ re-encoding, precoding and subtraction steps with a time-domain \gls{mrc} during each backscatter symbol block.
			}
			is desired to fully exploit signal characteristics and improve the primary-backscatter tradeoff.

			% Since scatter nodes directly modulate their message over the legacy signal, it involves a \emph{double modulation} where the primary and backscatter symbols are superimposed by \emph{multiplication coding} instead of superposition coding.
			...
			% Metascatter involves a symbiotic MAC where the primary and backscatter symbols of different duration are mixed by Multiplication Coding (MC) instead of Superposition Coding (SC). For each node, the reflection coefficient not only encodes the backscatter message, but also influences the equivalent primary channel (4). To accommodate such signal characteristics, novel receiving strategy apart from SIC is desired to better utilize the reflection pattern and boost the primary-backscatter tradeoff.
		\end{remark}
	...
	\begin{subsection}{RIScatter Nodes}
		...
		As illustrated in Fig. \ref{fg:scatter_comparison}, instead of always using fully random or deterministic reflection pattern over time, each RIScatter node can \emph{flexibly adjust} its input distribution to balance information encoding and channel reconfiguration.
