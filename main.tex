...
\begin{section}{Rate-Region Characterization}
	...
	\begin{subsection}{Decision Threshold}
		...
		\end{maxi!}
		which is still non-convex because variable $\boldsymbol{t}$ appear on the limits of integration \eqref{eq:dmtmac}.
		% % To tackle this, we 1) reveal backscatter decision design has no impact on primary decoding, 2) constrain the feasible domain of problem \eqref{op:decision_threshold} from continuous space $\mathbb{R}_+$ to finite candidate set (i.e., fine-grained discrete output energy levels) $\mathcal{T}$,
		% To tackle this, we 1) reveal backscatter decision design has no impact on primary decoding, 2) constrain the feasible domain of problem \eqref{op:decision_threshold} from continuous space $\mathbb{R}_+$ to finite candidate set of fine-grained discrete output energy levels $\mathcal{T}$, 3) regard UniScatter nodes with given input distribution as an equivalent augmented source in terms of total backscatter rate, 4) reuse existing quantization schemes for \gls{dmc} to solve problem \eqref{op:decision_threshold}.
		% % 3) indicate the equivalence between multi and single in terms of total backscatter rate
		% % 3) regard all UniScatter nodes as an equivalent augmented source in terms of total backscatter rate
		To tackle this, we first reveal backscatter decision design has no impact on primary decoding, then constrain the feasible domain of problem \eqref{op:decision_threshold} from continuous space $\mathbb{R}_+$ to finite candidate set (i.e., fine-grained discrete energy levels) $\mathcal{T}$.
		We also point out distributed nodes with given input distribution is equivalent to an augmented source of symbol tuples in terms of total rate, and reuse existing rate-optimal quantization schemes for \gls{dmc} to solve problem \eqref{op:decision_threshold}.
		...
		where $t_0^\star=0$ and $t_L^\star=\infty$ for energy-based backscatter detection.
