...
\begin{abstract}
	...
	% We uniquely introduce the concept of Metascatter, which adapts the input probability distribution of a finite-state backscatter device based on Channel State Information (CSI), to simultaneously act as parasitic source of Symbiotic Radio (SR) that conveys sensor data, and reflecting element of Intelligent Reflecting Surface (IRS) that assists legacy transmission.
	% We uniquely introduce the concept of Metascatter, which adjusts the input probability distribution of a finite-state backscatter device based on Channel State Information (CSI), to simultaneously act as parasitic source of Symbiotic Radio (SR) that conveys sensor data, and reflecting element of Intelligent Reflecting Surface (IRS) that assists legacy transmission.
	% We uniquely introduce the concept of Metascatter, which adjusts the input distribution of a finite-state backscatter device based on \gls{csi}, to \emph{simultaneously} encode its own data and assist ambient transmission.
	% We uniquely introduce the concept of Metascatter, which \emph{simultaneously} encodes own message and assists ambient transmission, using adaptive input probability distribution based on \gls{csi}.
	We uniquely introduce the concept of Metascatter, which adapts the input distribution of a finite-state backscatter device based on \gls{csi}, to \emph{simultaneously} encode self message and assist ambient transmission.
	% Compared to existing strategies as modulation on IRS pattern
	Compared to conventional \gls{irs}-empowered \gls{sr} that considers encoding and beamforming independently (overlay or parallel), Metascatter revisits backscatter principles and bridges parasitic source of \gls{sr} and reflecting element of \gls{irs} as extreme cases.



	% It enables smoother primary-backscatter tradeoff

	integrates \gls{irs} and \gls{sr} from backscatter principles

	% treat backscatter modulation and passive beamforming independently (overlay or distinction), Metascatter



	\gls{irs}-empowered \gls{sr} strategies that embed

	by adaptive input distribution design based on \gls{csi}.


	adjusts the input distribution of a finite-state backscatter device based on \gls{csi}, to \emph{simultaneously} encode its own data and assist ambient transmission.

	% It not only generalizes SR and IRS

	Compared to existing \gls{irs}-empowered \gls{sr},


	A flexible input distribution design

	Based on Channel State Information (CSI), the input distribution can be



	KKT tag input probability distribution for maximizing weighted sum of primary and total backscatter rate

	Using shared spectrum, energy, and infrastructures, fully cognitive networks can be built over Metascatter, and we consider a fundamental scenario where a multi-antenna Access Point (AP) and multiple Metascatter simultaneously transmit to a single-antenna user.
