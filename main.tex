...
\begin{section}{Backscatter Principles}
	...
	The structure of a passive tag is shown in Fig. \subref*{fi:passive_tag_structure}.
	...
	As illustrated by Fig. \subref*{fi:equivalent_model}, if $M$ symbol states are available at the tag, the reflection coefficient at state $m \in \mathcal{M} \triangleq \{1,\ldots,M\}$ is
	\footnote{
	...
	\end{equation}
	where $\alpha \in [0,1]$ models the (amplitude) harvest-backscatter efficiency at the direction of interest.
...
As such, the necessary optimality conditions for problem \eqref{op:decision_threshold}, $s \in \mathcal{S}$,
\begin{equation}
...
\end{equation}
where $l' \equiv s \pmod L$.
According to \cite{Nguyen2020a}, the optimal backward channel quantizer is convex and separates each pair of posterior distribution by a hyperplane.
It implies the sequence $\{\log {P(x_l \mid \hat{x}_{l'+1})}/{P(x_l \mid \hat{x}_{l'})}\}_{l \in \mathcal{L}}$ changes sign exactly once.
We notice the left-hand side of \eqref{eq:kkt_threshold_explicit} is a generalized Dirichlet polynomial, which has at most one real solution by Descartes' rule of signs \cite{Jameson2006}.
