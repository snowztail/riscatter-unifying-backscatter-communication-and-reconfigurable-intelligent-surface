...
\begin{section}{Simulation Results}
	...
	The finite threshold domain $\mathcal{T}$ comes from $b$-bit uniform discretization over the critical interval defined by the confidence bounds of edge hypotheses (i.e., lower bound of $\mathcal{H}_1$ and upper bound of $\mathcal{H}_L$) with confidence level $1-\varepsilon$, where $b=9$ and $\varepsilon=\num{1e-3}$.
	...
	\begin{subsection}{Evaluation of Proposed Algorithms}
		...
		\begin{subsubsection}{Convergence}
			...
			For $K=8$ and $M=2$, Algorithm \ref{al:input_distribution} typically takes around \num{100} fast iterations of closed-form expression \eqref{eq:input_kkt_solution} to converge to the \gls{kkt} input distribution.
			...
			In comparison, the \gls{bcd} algorithm typically requires no more than \num{5} iterations to converge.
			At the following points (not presented), the convergence of all three algorithms are much faster thanks to the progressive initialization.
	...
	\begin{subsection}{Comparison of Scattering Applications}
		...
		\begin{itemize}
			...
			\begin{equation}
				I_P^{\text{\gls{ambc}}}(s;y) \approx \log \Bigl(1 + \frac{\lvert\boldsymbol{h}_{\mathrm{D}}^H\boldsymbol{w}\rvert^2}{\sum_{k}\lvert \alpha_k \boldsymbol{h}_{\mathrm{C},k}^H \boldsymbol{w}\rvert^2+\sigma_v^2}\Bigr),
			\end{equation}
			while the total backscatter rate follows \eqref{eq:backscatter_mutual_information} with uniform input distribution.
			\item \emph{\gls{sr}:}\footnote{Existing } For a sufficiently large $N$, the primary ergodic rate under semi-coherent detection can be approximated by \eqref{eq:primary_mutual_information} with uniform input distribution \cite{Long2020a}.
			...
			\end{equation}
			which, under hypothesis $\mathcal{H}_{m_{\mathcal{K}}}$, is only subject to the noise uncertainty and follows non-zero mean complex Gaussian distribution $\mathcal{CN}\bigl(\sum_{k} \alpha_k \boldsymbol{h}_{\mathrm{C},k}^H x_k \boldsymbol{w} s[n],\sigma_v^2\bigr)$.
			Energy detection is generally suboptimal in this case
			% By the law of large numbers, for a very large $N$, the \gls{pdf} of accumulated intermediate energy approaches
			% \begin{equation}
			% 	f^{\text{\gls{sr}}}(z|\mathcal{H}_{m_{\mathcal{K}}}) = \frac{(z-\mu_{m_{\mathcal{K}}}^{\text{\gls{sr}}})^{N-1} \exp \bigl(-(z-\mu_{m_{\mathcal{K}}}^{\text{\gls{sr}}})/\sigma_v^2 \bigr)}{\sigma_v^{2N} (N-1)!},
			% 	\label{eq:energy_distribution_sr}
			% \end{equation}
			% where $\mu_{m_{\mathcal{K}}}^{\text{\gls{sr}}} \triangleq N \bigl\lvert \sum_{k} \alpha_k \boldsymbol{h}_{\mathrm{C},k}^H x_{m_k} \boldsymbol{w} \bigr\rvert^2$. The \gls{ml} decision threshold is, $\forall l \in \mathcal{L} \setminus \{L\}$,
			% \begin{equation}
			% 	t_l^{\text{\gls{sr}}} = \frac{\mu_{l-1}^{\text{\gls{sr}}} \exp \bigl((\mu_{l-1}^{\text{\gls{sr}}}-\mu_{l}^{\text{\gls{sr}}})/\sigma_v^2 (N-1)\bigr) - \mu_{l}^{\text{\gls{sr}}}}{\exp \bigl((\mu_{l-1}^{\text{\gls{sr}}}-\mu_{l}^{\text{\gls{sr}}})/\sigma_v^2 (N-1)\bigr) - 1}.
			% 	\label{eq:detection_threshold_ml_sr}
			% \end{equation}
		\end{itemize}
	\end{subsection}
