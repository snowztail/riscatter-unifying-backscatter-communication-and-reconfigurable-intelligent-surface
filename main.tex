...
\begin{document}
	...
	\begin{section}{System Model}
		...
		\begin{subsection}{Backscatter Transmission}
			...
			\end{equation}
			denotes the typical energy of the received signal when the tag is at state $m$, which can be estimated at the user. For the ease of exposition, we sort $\{\sigma_m^2\}$ in a descending order by bijective mapping $f \colon m \mapsto i$,\footnote{Note that the mapping is not unique and the detection fails when two different constellation points yield the same energy level.} and formulate a sequence $\langle \sigma_i^2 \rangle$ where $\sigma_i^2 \ge \sigma_j^2$ for $i > j$, $i,j \in \mathcal{M}$. Denote $\mathcal{H}_i$ as the hypothesis that the tag transmits $c_i$. Following \cite{Qian2019}, the optimal Maximum-Likelihood (ML) detector boils down to the energy detector, and the likelihood ratio between hypotheses $\mathcal{H}_i$ and $\mathcal{H}_j$ is
			\begin{equation}
			...
			\end{equation}
			where $T_{0, 1} \triangleq 0$ and $T_{M, M+1} \triangleq \infty$. On top of this, we define the forward transition probability matrix as
			\begin{equation}
			...
			\end{equation}
			where $\Gamma(\cdot)$ is the gamma function. To summarize, once the CSI of the direct and cascaded links are obtained, the typical energy of the received signal can be estimated by \eqref{eq:typical_energy}, the energy detection threshold can be computed by \eqref{eq:detection_threshold}, the decision region can be retrieved by \eqref{eq:decision_region}, and the forward transition matrix $\boldsymbol{P}$ can be calculated by substituting \eqref{eq:conditional_pdf} into \eqref{eq:transition_probability}.
			...
			\begin{equation}
				R_c = \max_{\boldsymbol{Q}} \sum_{i \in \mathcal{M}} \sum_{j \in \mathcal{M}} r_i p_{i, j} \log \frac{q_{j, i}}{r_i},
			\end{equation}
			where $\boldsymbol{Q}$ is the backward transition probability matrix \cite{Blahut1972}
			\begin{equation}
		...
		\begin{subsection}{Legacy Transmission}
			...
			\begin{align}
				...
				& = \sum_{m \in \mathcal{M}} r_m \log_2 \bigl( 1 + \gamma \lvert h_{\mathrm{D}} + \sqrt{\alpha} h_{\mathrm{C}} c_m \rvert^2 \bigr),
			\end{align}
			where $\gamma \triangleq P / \sigma^2$ is the average transmit Signal-to-Noise Ratio (SNR). It can be observed that the input distribution of $c$ determines the achievable rate of both primary and secondary links.
		\end{subsection}
	...
	\begin{section}{Tag Input Distribution}
		...
		\begin{maxi!}
			{\scriptstyle{\boldsymbol{r}}}{\rho R_s + (1 - \rho) R_c}{}{}
			\addConstraint{\sum_{m \in \mathcal{M}} r_m}{=1,}{}
		\end{maxi!}
		where $\rho \in [0, 1]$ is the weight factor.
	\end{section}
