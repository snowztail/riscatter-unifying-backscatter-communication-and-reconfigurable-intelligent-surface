...
\begin{document}
	...
	\begin{section}{The Multi-Tag Case}
		...
		\begin{subsection}{Backscatter Detection}
			...
			\end{equation}
			where $\Gamma(\cdot)$ is the gamma function. As such, the equivalent discrete memoryless channel can be defined by a transition matrix where the probability of observing output $j \in \mathcal{M^K}$ under input $i \in \mathcal{M^K}$ is\footnote{For simplicity, we assume all possible input combinations yield distinct energy levels.}
			\begin{equation}
