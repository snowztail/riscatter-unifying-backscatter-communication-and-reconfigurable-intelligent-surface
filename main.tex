...
\begin{section}{Simulation Results}
	...
	\begin{subsection}{Comparison of Scattering Applications}
		% AmBC: co-located non-cooperative receivers, use SINR to evaluate primary rate even if backscatter symbols are not Gaussian distributed
		% For a fair comparison, we employ energy-based backscatter detection for scattering applications evaluated in this subsection.
		% For a fair comparison, we evaluate different scattering applications with backscatter energy detection over the setup in Fig. \ref{fi:riscatter_network}.
		...
		% employ energy-based backscatter detection for scattering applications evaluated in this subsection.
		\begin{itemize}
			\item \emph{\gls{bbc}:}
			% The \gls{ap} becomes carrier emitter and the user becomes backscatter reader.
			...
			\end{equation}
			% which follows non-zero mean complex Gaussian distribution $y_{m_{\mathcal{K}}}^{\text{BBC}}[n] \sim \mathcal{CN}\bigl(\boldsymbol{h}_{\mathrm{E}}^H(x_{m_{\mathcal{K}}}) \boldsymbol{w},\sigma_v^2\bigr)$ under hypothesis $\mathcal{H}_{m_{\mathcal{K}}}$.
			...
			% Hence, the primary achievable rate can be approximated by\footnote{To provide a preliminary benchmark, we approximate the interference from finite-input backscatter sources by those from independent Gaussian sources.}
