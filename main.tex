...
\newacronym{mmse}{Minimum Mean-Square-Error}{MMSE}
...
\begin{abstract}
	...
	% \gls{bc} devices vary load impedance to modulate and reflect the signals
	% \gls{bc} nodes rely on dedicated carrier wave to
	% \gls{bc} nodes manipulate the magnitude, phase, and/or frequency responses to harvest energy from and modulate information over the carrier wave from a nearby emitter.
	\gls{bc} nodes harvest energy from and modulate information over an external carrier by manipulating the magnitude, phase, and/or frequency of the scattered signal.
	...
	RIScatter is a new paradigm for future wireless networks and consists of multiple dispersed or co-located passive scattering nodes, whose reflection states can be adapted to partially engineer the wireless channel and partially modulate information onto the scattered wave.
	...
	This enables RISscatter to softly bridge, generalize, and outperform \gls{bc}, \gls{sr}, \gls{ris}; boil down to any of those under specific reflection states; or evolve in a mixed form for universal hardware design and heterogeneous traffic control.
	% ? We show RIScatter provides a fundamental tradeoff that balances information modulation and channel reconfiguration capabilities in a flexible and mutualistic manner.
	...
	For a typical setup, we characterize the achievable primary-(total-)backscatter rate region by optimizing input distribution at the nodes, active beamforming at the \gls{ap}, and backscatter detector at the user.
	% An application scenario is considered where a multi-antenna \gls{ap} serves a single-antenna user surrounded by dispersed RIScatter nodes, and the achievable primary-total-backscatter rate region is characterized by optimizing input distribution at the nodes, active beamforming at the \gls{ap}, and backscatter detector at the user.
	...
	Simulation results demonstrate RIScatter nodes can exploit the additional propagation paths to smoothly transition between backscatter modulation and passive beamforming via smart input distribution design,
	...
	and the proposed practical receiver effectively accommodates the double modulation and signal characteristics in scatter-based cooperative networks.
	% with low complexity.
\end{abstract}
...
\begin{section}{Introduction}
	\begin{subsection}{Fundamentals}
		...
		In particular, the node harvests energy from impinging wave and embeds information over scattered signal, while the backscatter reader can be either co-located or separated with the carrier emitter, shown as \gls{mbc} and \gls{bbc} in Fig. \subref*{fg:mbc} and \subref*{fg:bbc}, respectively.
		...
		To tackle this, \cite{Liu2013b} proposed an \gls{ambc} system where interactive nodes recycle ambient legacy signals (e.g., radio, television and Wi-Fi) to harvest energy and establish connections, and the idea is shown in Fig. \subref*{fg:ambc}.
		...
		In \cite{Yang2018}, the authors proposed a cooperative \gls{ambc} system where both primary (legacy) and backscatter links are decoded at the same user under various detection schemes.
		% a cooperative receiver decodes both primary (legacy) and backscatter links using \gls{ml}, \gls{mrc}, \gls{zf}, \gls{mmse} and \gls{sic}
		% co-located cooperative receiver to jointly or sequentially decode both primary (legacy) and backscatter links.
		...
		The concept of cooperative \gls{ambc} was further refined as \gls{sr} that cognitively incorporates \gls{ambc} with existing systems \cite{Liang2020}, as shown in Fig. \subref*{fg:sr}.
		% shown in Fig. \subref*{fg:sr}, which cognitively incorporates \gls{ambc} with existing systems \cite{Liang2020}.
		...
		% An illustration of \gls{sr} is shown in \subref*{fg:sr}, where the primary transmitter generates active radio and modulates primary information, the passive node scatters incoming wave and superimposes backscatter information, and the co-located receiver decodes both links from both propagation paths.
		In \gls{sr}, the active transmitter generates \gls{rf} wave carrying primary information, the passive node enhances the radio propagation and superimposes its own message on the scattered signal, and the cooperative receiver jointly or sequentially decodes both links.
		...
		% The authors concluded if this ratio is sufficiently large, the noncoherent primary achievable rate would asymptotically approach its coherent counterpart.
		...
		% However, the advantage of \gls{sic} is questionable because 1) sufficiently large symbol period ratio is assumed in primary rate analysis and constraints backscatter rate; 2) backscatter pattern and signal characteristics are not fully exploited; 3) noncoherent primary encoding is required at the transmitter, while re-encoding, precoding, and subtraction are required at the receiver; 4) primary and backscatter symbols are superimposed by multiplication instead of superposition.
		...
		% Above applications treat the scattering node as an information source.
		...
		The reflection pattern is deterministic over time, which can be optimized and reported to the user before the transmission.
		...
		% in Fig. \subref*{fg:ris} is a promising technology to recycle
		% On the other hand, \gls{ris} in Fig. \subref*{fg:ris} is a promising technology that evolves wireless propagation environment using numerous passive reflecting elements (e.g., scattering antenna or programmable metamaterial) with adjustable amplitudes and/or phases \cite{Wu2021b}.
		...
		% Compared with scattering nodes of \gls{bc}/\gls{sr}, \gls{ris}
		% elements embed no additional information and use deterministic reflection pattern over time, which can be optimized and fed back to the transmitter and receiver during coordination phase.
	...
	\begin{subsection}{Related Works}
		...
		% To evaluate the performance of cooperative receivers, \cite{Yang2018} derived the bit error rates of \gls{ml}, \gls{mrc}, \gls{zf}, \gls{mmse} and \gls{sic} detectors for flat fading channels, and proposed a low-complexity detector for frequency-selective fading channels.
		...
		% However, it assumed the primary and backscatter symbol periods are equal and
		% However, it only considered the case where primary and backscatter symbols have the same period.
		...
		Joint \gls{ml} decoding achieves the best error performance in general, but its computational complexity can be prohibitive especially for sources with high-order constellation \cite{Yang2018,Liang2020,Zhang2022}.
		One special property of scatter-based cooperative networks is the backscatter signal strength is significantly weaker than primary due to the double fading effect.
		It motivated \cite{Long2020a,Liang2020,Guo2019b,Ding2020,Zhou2019a,Wu2021a,Xu2021a,Yang2021a,Yang2018,Han2021,Zhang2022} to view \gls{sr} as a multiplicative \gls{noma} and perform sequential \gls{sic}-based primary-backscatter decoding.
		...
		% During primary decoding, the randomness from backscatter modulation can be modelled as either interference or channel uncertainty, depending on the relationship between backscatter switching speed and primary sampling rate.
		% If the backscatter-over-primary symbol period ratio $N$ is sufficiently large (i.e., commensal \gls{sr}), then the average primary achievable rate under noncoherent detection would asymptotically approach its coherent counterpart \cite{Long2020a}, and both links may be decoded in an interference-free manner.
		If the former is much higher (i.e., commensal \gls{sr}), then the average primary achievable rate under noncoherent detection would asymptotically approach its coherent counterpart \cite{Long2020a}, and both links may be decoded in an interference-free manner.
		...
		% If this ratio is sufficiently large, the authors proved the noncoherent primary achievable rate would approach its coherent counterpart asymptotically.
		...
		% However, the disadvantage of \gls{sic}-based sequential detection includes:
		% 1) the achievable primary and backscatter rate analysis for commensal \gls{sr} only holds for a sufficiently large $N$;
		% 1) the asymptotic rate analysis for commensal \gls{sr} is inapplicable when primary and backscatter symbol periods are comparable; \footnote{For example, Wi-Fi sampling rate is \qty{20}{\MHz} while \gls{rfid} load switching speed varies between \num{100}s of \unit{\kHz} to \num{10}s of \unit{\MHz} \cite{Torres2021}, corresponding to a typical symbol period ratio between \num{1} and \num{100}.}
		% ! However, the disadvantage of \gls{sic}-based sequential detection includes:
		% 1) the asymptotic rate analysis for commensal \gls{sr} is inapplicable when symbol periods are comparable \footnote{For example, Wi-Fi sampling rate is \qty{20}{\MHz} while \gls{rfid} load switching speed varies between \num{100}s of \unit{\kHz} to \num{10}s of \unit{\MHz} \cite{Torres2021}, corresponding to a typical symbol period ratio between \num{1} and \num{100}.};
		% 2) using a large symbol period ratio not only reduces backscatter throughput, but also requires \gls{sic} over every primary block
		However, it requires \gls{sic} over all primary symbol blocks followed by a time-domain \gls{mrc}, which is
		% computationally expensive
		operation-intensive and \gls{csi}-sensitive.
		Besides, the assumption of very large backscatter-over-primary symbol period ratio not always hold in modern wireless systems \footnote{For example, Wi-Fi sampling rate is \qty{20}{\MHz} while \gls{rfid} load switching speed varies between \num{100}s of \unit{\kHz} to \num{10}s of \unit{\MHz} \cite{Torres2021}, corresponding to a typical symbol period ratio between \num{1} and \num{100}.}.

		% the assumption of long backscatter symbol constrains the backscatter data rate\footnote{For example, Wi-Fi sampling rate is \qty{20}{\MHz} while \gls{rfid} load switching speed varies between \num{100}s of \unit{\kHz} to \num{10}s of \unit{\MHz} \cite{Torres2021}, corresponding to a typical (backscatter-over-primary) symbol period ratio between \num{1} and \num{100}.}
		...
		primary rate analysis is backed by long backscatter symbol period that constrains backscatter rate;
		2) reflection pattern and signal characteristics are not fully exploited;
		3) noncoherent primary encoding is required at the transmitter while re-encoding, precoding and subtraction are required at the receiver;
	...
	\begin{subsection}{Contributions}
		...
		The paradigm of RIScatter is presented in Fig. \subref*{fg:riscatter}, and the contributions of this paper are summarized as follows.
...
\begin{section}{Scattering Principles}
	...
	\begin{subsection}{Opportunities}
		...
		This question is answered in Section \ref{st:riscatter}.
	\end{subsection}
\end{section}
...
\begin{section}{RIScatter}
	...
	\begin{subsection}{RIScatter Nodes}
		...
		\begin{figure}[!t]
			...
			\label{fg:riscatter_distribution}
		\end{figure}
		As illustrated in Fig. \ref{fg:riscatter_distribution}, instead of always using fully random or deterministic reflection pattern over time, each RIScatter node can \emph{flexibly adjust} its input distribution to balance information encoding and channel reconfiguration.
		...
		As shown in Fig. \subref*{fg:block_diagram}, RIScatter nodes can be implemented over readily available passive backscatter devices with energy harvester and information decoder.
		Its equivalent circuit and scatter model are presented in Fig. \subref*{fg:equivalent_circuit} and \subref*{fg:scatter_model}.
	...
	\begin{subsection}{RIScatter Receiver}
		...
		\end{figure}
		As illustrated in Fig. \ref{fg:energy_distribution}, the total received energy per backscatter block is a random variable that follows different distributions conditioned on different input state hypotheses.
	...
	\begin{subsection}{System Model}
		...
		\end{figure}
		To demonstrate the advantages of RIScatter, we consider a single-user multi-node \gls{miso} RIScatter system as shown in Fig. \ref{fg:riscatter_network}.
		...
		As illustrated in Fig. \ref{fg:energy_distribution}, the RIScatter receiver divides the received energy space into disjoint decision regions associated with those hypotheses.
...
\begin{section}{Rate-Region Characterization}
	...
	\begin{subsection}{Decision Threshold}
		...
		\end{figure}
		As shown in Fig. \ref{fg:discrete_outputs}, by introducing the extra analog-to-digital conversion step, we can group adjacent high-resolution energy bins for backscatter decision regions.
		...
		In Section \ref{st:simulation_results}, both schemes will be compared with the \gls{ml} thresholding \cite{Qian2019}
		\begin{equation}
...
\begin{section}{Simulation Results}
	...
	\begin{subsection}{Comparison of Scattering Applications}
		...
		% For a fair comparison, we consider the setup in Fig. \ref{fg:riscatter_network} and employ \gls{ml} backscatter energy detection for these scattering applications:
		% For a fair comparison, we consider the setup in Fig. \ref{fg:riscatter_network} with \gls{ml} backscatter energy detection for these scattering applications (except \gls{sr}):
		% For a fair comparison with RIScatter, we consider the setup in Fig. \ref{fg:riscatter_network} and evaluate the achievable rate pairs for these scattering applications:
		% We consider the setup in Fig. \ref{fg:riscatter_network} and compare the achievable rate pairs of RIScatter and these scattering applications:
	...
	% 	\begin{figure}[!t]
	% 		\subfloat[Primary Rate\label{fg:rate_bits_primary}]{
	% 			\resizebox{0.48\columnwidth}{!}{
	% 				\input{assets/simulation/rate_bits_primary.tex}
	% 			}
	% 		}
	% 		\subfloat[Total Backscatter Rate\label{fg:rate_bits_backscatter}]{
	% 			\resizebox{0.48\columnwidth}{!}{
	...
	% 	For a fixed input distribution and active beamforming, Fig. \ref{fg:rate_bits} shows the impact of energy discretization bits $b$ on the average achievable rates when $\rho=0$.
	...
	\begin{subsection}{Rate Region by Different Schemes}
		% \begin{figure}[!t]
		% 	\centering
		% 	\subfloat[Input Distribution\label{fg:region_distribution}]{
		% 		\resizebox{0.65\columnwidth}{!}{
		% 			\input{assets/simulation/region_distribution.tex}
		% 		}
		% 	}
		% 	\\
		% 	\subfloat[Active Beamforming\label{fg:region_beamforming}]{
		% 		\resizebox{0.65\columnwidth}{!}{
		% 			\input{assets/simulation/region_beamforming.tex}
		% 		}
		% 	}
		% 	\\
		% 	\subfloat[Decision Threshold\label{fg:region_threshold}]{
		% 		\resizebox{0.65\columnwidth}{!}{
		...
		\begin{subsubsection}{Input Distribution}
			...
			Fig. \subref*{fg:region_distribution} shows the achievable rate regions for those input distribution schemes.
		...
		\begin{subsubsection}{Active Beamforming}
			...
			Fig. \subref*{fg:region_beamforming} plots the achievable rate regions for those active beamforming schemes.
		...
		\begin{subsubsection}{Decision Threshold}
			...
			Fig. \subref*{fg:region_threshold} presents the achievable rate region for those decision threshold schemes.
	...
	\begin{subsection}{Rate Region under Different Configurations}
		...
		\begin{subsubsection}{Number of Nodes}
			Fig. \subref*{fg:region_tags} reveals how the number of two-state RIScatter nodes $K$ influence the primary-backscatter tradeoff.
		...
		\begin{subsubsection}{Coverage Disk Radius}
			Fig. \subref*{fg:region_coverage} shows the relationship between RIScatter cover disk radius and achievable rate regions.
