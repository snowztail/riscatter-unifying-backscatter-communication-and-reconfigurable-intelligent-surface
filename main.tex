...
\begin{document}
	...
	\begin{section}{Backscatter Detection and Achievable Rates}
		...
		\begin{subsection}{Backscatter Detection}
			...
			\begin{theorem}
				If any $L$-input continuous-output channel follows Erlang distribution~\eqref{eq:energy_distribution}, then the optimal number of thresholds is $L+1$ and the capacity-achieving decision region $j$ has convex structure $\mathcal{R}_j^\star = [t_{j-1}^\star, t_j^\star)$.
			\end{theorem}

			\begin{proof}
				For any finite number of thresholds $S \ge L+1$, let $\boldsymbol{t} \triangleq [t_0,\ldots,t_S]^T \in \mathbb{R}_{+}^{S \times 1}$ be the thresholding vector where $t_{s-1} < t_s$, $\forall s \in \mathcal{S} \triangleq \{1,\ldots,S\}$.\footnote{We consider strict inequality in the definition, as otherwise $S$ and $\boldsymbol{t}$ could be non-unique for a given thresholding scheme.} Denote the decision partition $s$ as $R_s \triangleq [t_{s-1},t_s)$. Without loss of generality, we assume
				% $L \mid S$ and
				...
				\begin{maxi!}
					{\scriptstyle{\boldsymbol{t}}}{I_{\mathrm{B}}(c_{\mathcal{K}};z)}{\label{op:thresholding}}{\label{ob:backscatter_mutual_information}}
				...
				\begin{equation}
					...
					% = \sum_{j \in \mathcal{L}} P(\bar{z}_j) \log P(\bar{z}_j) - \sum_{i \in \mathcal{L}} \sum_{j \in \mathcal{L}}
				\end{equation}
				...
				\end{subequations}
				Due to the strict inequality constraint~\eqref{co:strict_inequality}, conditions \eqref{eq:dual_feasibility} and \eqref{eq:complementary_slackness} together imply $\mu_s^\star = 0$, $\forall s \in \mathcal{S}$. For \gls{dmtc} based on energy detection~\eqref{eq:energy_distribution}, it is trivial to conclude $t_0^\star = 0$ and $t_S^\star = \infty$. Hence, we focus on $s' \in \mathcal{S} \setminus \{S\}$ and \eqref{eq:kkt_thresholding} reduces to
				\begin{equation}
					\nabla_{t_{s'}^\star} I^\star_{\mathrm{B}}(c_{\mathcal{K}};z) = 0,
				\end{equation}
				that is, for $j \in \mathcal{L}$ and $j \equiv s' \pmod L$,
				\begin{equation}
				...
				Since the optimal backward channel quantizer is convex and separates each pair of posterior distribution by a hyperplane~\cite{Nguyen2020a}, the sequence $(a_{l,j})_{l=1}^{L}$ only change sign once for a given $j$. Besides, we notice the LHS of~\eqref{eq:kkt_thresholding_explicit} is a generalized Dirichlet polynomial, as $f(t_{s'}^\star \mid x_l)$ follows Erlang distribution~\eqref{eq:energy_distribution}. By Descartes' rule of signs~\cite{Jameson2006}, for each $j \in \mathcal{L}$, $t_s^\star$ has at most 1 real solution if $\sigma_l^2 \ne \sigma_{l'}^2$, $l,l' \in \mathcal{L}$.

				% if $\sigma_l^2 \ne \sigma_{l'}^2$, $l,l' \in \mathcal{L}$, then $t_s^\star$ has at most 1 real solution.

				% for a given $j$, $t_s^\star$ has at most 1 real solution if $\sigma_l^2 \ne \sigma_{l'}^2$, $l,l' \in \mathcal{L}$.

				% Therefore,

				% it can be concluded that for a given $j$, the sequence $(a_{l,j})_{l=1}^{L}$ only change sign once. Plugging~\eqref{eq:energy_distribution} into~\eqref{eq:kkt_thresholding_explicit}, we have
				% \begin{equation}
				% 	\sum_l a_{l,j} e^{-{t_s^\star}/{\sigma_l^2}} = 0,
				% 	\label{eq:optimal_thresholding_condition}
				% \end{equation}
				% where the LHS is a generalized Dirichlet polynomial. By Descartes' rule of signs~\cite{Jameson2006}, for a given $j$, $t_s^\star$ has at most 1 real solution if $\sigma_l^2 \ne \sigma_{l'}^2$, $l,l' \in \mathcal{L}$.

				% he number of positive roots is at most the number of sign changes in the sequence of polynomial's coefficients
				% and by Descartes' rule of signs has at most $L-1$ distinct real roots if $\sigma_i^2 \ne \sigma_j^2$, $i,j \in \mathcal{L}$.


				% % As proved by~\cite{Nguyen2020a}, the optimal backward channel quantizer
				% % \begin{equation}
				% % 	\sum_l P(x_l) \log \frac{P(z_j)}{P(z_{j+1})} \frac{P(z_{j+1} \mid x_l)}{P(z_j \mid x_l)} f(t_s^\star \mid x_l) = 0,
				% % \end{equation}
				% where $j \equiv s \pmod L$. Since the
				% % We notice that for a given $j$, $a_l \triangleq P(x_l) \log \frac{P(z_j)}{P(z_{j+1})} \frac{P(z_{j+1} \mid x_l)}{P(z_j \mid x_l)}$ is a constant for all $s \equiv j \pmod L$. Plugging~\eqref{eq:energy_distribution} into \eqref{eq:kkt_thresholding_explicit}, we have
				% \begin{equation}
				% 	\sum_l a_{l,j} e^{-{h_s}/{\sigma_l^2}} = 0,
				% 	\label{eq:optimal_thresholding_condition}
				% \end{equation}
				% where the LHS is a Dirichlet polynomial and by Descartes' rule of signs has at most $L-1$ distinct real roots if $\sigma_i^2 \ne \sigma_j^2$, $i,j \in \mathcal{L}$.
				% % where the directional derivative can be explicitly written as
				% % \begin{subequations}
				% % 	\begin{align}
				% % 		\nabla_{t_{s'}^\star} I^\star_{\mathrm{B}}(c_{\mathcal{K}};z)
				% % 		& = \sum_l P(x_l) \log \frac{P(z_j)}{P(z_{j+1})} \frac{P(z_{j+1} \mid x_l)}{P(z_j \mid x_l)} f(h_s \mid x_l)\\
				% % 		& = 1
				% % 	\end{align}
				% % \end{subequations}
			\end{proof}

			% It was proved in \cite{Nguyen2021} that the optimal quantizer producing maximizing mutual information with arbitrary cost function separates the backward channel (i.e., posterior distribution) in a convex manner

			% not only the capacity-achieving thresholding design remains an open problem, but also the optimal number of thresholds for general non-binary-input channels remains unknown. I am now trying to prove that, for our channel in Gamma distribution, the optimal decision region for each codeword cannot be disjoint
	...
	\begin{section}{Input Distribution, Decision Region, and Precoder Design}
		\begin{subsection}{Primary-backscatter rate region}
			...
			\begin{maxi!}
				...
				\addConstraint{\sum \nolimits_{m_k} P_k(\bar{c}_{m_k})}{=1,}{\quad \forall k \in \mathcal{K}}{\label{co:sum_probability}}
				\addConstraint{P_k(\bar{c}_{m_k})}{\ge 0,}{\quad \forall m_k \in \mathcal{M}, \ \forall k \in \mathcal{K}}{\label{co:nonnegative_probability}}
