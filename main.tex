...
\begin{document}
	...
	\begin{section}{The Multi-Tag Case}
		\begin{subsection}{System Model}
			...
			\begin{remark}
				The proposed symbiotic radio system is reminiscent of a Multiple Access Channel (MAC) where the AP and the tags transmit simultaneously to the user. It inspired all existing papers [TODO] to perform Successive Interference Cancellation (SIC) that first non-coherently detects the primary message under backscatter uncertainty (requires non-coherent codes), then cancels its contribution and decodes the backscatter messages. However, different from the conventional MAC with Superposition Coding (SC), the symbiotic radio system involves Multiplication Coding (MC) because the primary and secondary messages are indeed combined in the received signal by multiplication. Also, the computational complexity of SIC can be high since it requires $K$ re-encoding, precoding, and subtraction operations. Therefore, novel multi-stream detection techniques should be tailored for symbiotic radio systems.
			\end{remark}
		\end{subsection}
