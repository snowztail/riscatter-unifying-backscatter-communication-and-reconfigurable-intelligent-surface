...
\begin{abstract}
	...
	For simplicity, it is assumed the user first jointly decodes all backscatter messages based on average received power, then models reflection patterns and backscatter paths within equivalent channel for primary decoding.
...
\begin{section}{Introduction}
	...
	\cite{Zhou2019a} explored the impact of transmit antennas, receive antennas, and backscatter symbol period on the ergodic rate of primary and backscatter links.
	...
	However, those paper only considered single backscatter node and backscatter multiple access remains an open issue.
	In \cite{Xu2021a}, a \gls{noma}-based \gls{sr} was proposed and receive combining was investigated for \gls{sic} order in equivalent channel strength.
	...
	A \gls{tdma}-based \gls{sr} with energy harvesting constraints was also presented in \cite{Yang2021a}, where transmit power, reflection efficiency, and time allocation was optimized to maximize energy efficiency.
	...
	Extensive research has been devoted to optimizing the phases of \gls{irs} during whole channel block for different metrices, as TODO \cite{Wu2018,Wu2019a,Wu2019}.
	...
	Besides, dynamic passive beamforming that adjusts \gls{irs} over fine-grained slots within channel coherence time was proposed for \gls{ofdm} systems in \cite{Yang2020}.

	The idea was further applied to \gls{wpcn} to strike the

	proactively generate favourable time-varying channels by properly designing the IRS phase-shift vectors over different time slots,
	which thus enhances the multiuser diversity over time and also allows more flexible resource allocation.

	mic IRS beamforming is actually beneficial for maximizing the throughput
	of WPCNs
	% The authors
	% of [29] considered a novel dynamic passive beamforming for
	% Orthogonal Frequency-Division Multiplexing (OFDM) systems,
	% where the reflection coefficient is varied over consequent
	% time slots to enable flexible resource allocation over timefrequency Resource Blocks (RBs).

	further adjusted the IRS reflection pattern within channel coherence time to mimic multi-beam reflection in a time-division manner
