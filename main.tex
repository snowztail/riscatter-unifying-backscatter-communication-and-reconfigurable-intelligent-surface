...
\begin{section}{Introduction}
	...
	\begin{subsection}{Contributions}
		...
		Scattering sources of \gls{bc}/\gls{sr} and reflecting elements of \gls{ris} can be regarded as special cases with uniform and degenerate input distributions, respectively.
...
\begin{section}{RIScatter}
	As a generalization of \gls{bc}, \gls{sr} and \gls{ris}, RIScatter is a passive scattering protocol that rides over an active primary link in a flexible and mutualistic manner.
	Specifically, RIScatter nodes utilize scattered multipath to smoothly transition between backscatter modulation and passive beamforming by \gls{csi}- and weight-based input distribution design, and a tailored cooperative receiver exploits signal properties (i.e., double modulation and symbol period difference) to effectively decode both primary and backscatter links.
	RIScatter nodes can be implemented by adding an adaptive encoder to off-the-shelf passive backscatter tags with an integrated receiver, and the cascaded \gls{csi} acquisition

	acquisition depends on the geometry of nodes.
	For co-located RIScatter nodes,
	...
	can be acquired using sequential methods \cite{Bharadia2015,Yang2015b,Guo2019g} or the state-of-the-art parallel approach \cite{Jin2021a}.
