...
\begin{section}{Metascatter-Enabled Network}
	\begin{subsection}{System Model}
		...
		Under perfect synchronization, the signal received by the user at primary block $n \in \mathcal{N} \triangleq \{1,\ldots,N\}$ is
		\begin{equation}
			y[n] = \left(\boldsymbol{h}_{\mathrm{D}}^H + \sum_{k \in \mathcal{K}} \sqrt{\alpha_k} \boldsymbol{h}_{\mathrm{C},k}^H x_k\right) \boldsymbol{w} s[n] + v[n],
			\label{eq:received_signal}
		\end{equation}
		where $\alpha_k$ is the amplitude reflect ratio of Metascatter $k$, $x_k \in \mathcal{X} \triangleq \{\Gamma_1,\ldots,\Gamma_M\}$ is the backscatter symbol of Metascatter $k$, $s \sim \mathcal{CN}(0,1)$ is the primary symbol, $v \sim \mathcal{CN}(0,\sigma_v^2)$ is the \gls{awgn}, $\boldsymbol{w} \in \mathbb{C}^{Q \times 1}$ is the active beamforming vector satisfying the average power constraint $\lVert \boldsymbol{w} \rVert^2 \le P$.
		...
		\end{equation}
		where $x_{\mathcal{K}} \triangleq \{x_k : k \in \mathcal{K}\}$.
