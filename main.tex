...
\begin{section}{Backscatter Principles}
	...
	As shown in Fig. \subref*{fi:block_diagram}, a typical passive node consists of a scattering antenna, an energy harvester, an integrated receiver, a load-switching modulator, and on-chip components (e.g., microcontroller and sensors) \cite{Dobkin2012}.
	Its equivalent circuit is presented in Fig. \subref*{fi:equivalent_circuit}.
	When illuminated, the node absorbs a portion of the impinging wave for information decoding and energy harvesting \cite{Kim2021a}, and scatters the remaining to the space. The scattered signal is further decomposed into the \emph{structural} and \emph{antenna} mode components \cite{Hansen1989}.
	...
	Fig. \subref*{fi:scatter_model} illustrates the scatter model of a node with $M$ reflection states, where the reflection coefficient of state $m \in \mathcal{M} \triangleq \{1,\ldots,M\}$ is defined as%
	...
	\begin{subsection}{Backscatter Modulation}
		...
		% To perform backscatter modulation, nodes encode message within distribution of reflection states.%
		% To perform backscatter modulation, nodes encode message within distribution of reflection states.%
		Backscatter nodes encode message by frequently change its reflection state.

		% Although most backscatter and \gls{sr} literatures assumed equiprobable state distribution at nodes, the
