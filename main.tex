...
\begin{document}
	...
	\begin{section}{The Multi-Tag Case}
		...
		\begin{subsection}{Primary ergodic capacity}
			Once the tag symbol set $\bar{c}_{m_{\mathcal{K}}}$ is successfully decoded, the user can eliminate the backscatter uncertainty and model it within the equivalent primary channel by \eqref{eq:equivalent_primary_channel}. From the perspective of primary transmission, the tags adjust the propagation environment in a potentially beneficial manner, and create artificial channel variation within each fading block. As such, the ergodic primary capacity can be expressed as
			\begin{align}
				R_s
	...
	\begin{section}{Tag Input and Precoder Design}
		\begin{subsection}{Primary ergodic-secondary sum rate region}
			To achieve a flexible tradeoff between the primary and backscatter links, we aim to optimize the precoder and tag input distribution to maximize the weighted sum of the ergodic primary rate and the total backscatter rate, under the average transmit power and probability constraints
			\begin{maxi!}
				{\scriptstyle{{P(c_{\mathcal{K}})},{\boldsymbol{w}}}}{\rho R_s + (1 - \rho) R_c}{\label{op:rate_region}}{\label{ob:weighted_sum_rate}}
			...
			\end{maxi!}
			where $\rho \in [0,1]$ denotes the priority of the primary link. As problem \eqref{op:rate_region} is not jointly convex over $\boldsymbol{w}$ and $P(c_{\mathcal{K}})$, we propose an alternating optimization (AO)-based algorithm that iteratively retrieve the global optimal precoder and capacity-achieving input distribution in closed forms.
		\end{subsection}
