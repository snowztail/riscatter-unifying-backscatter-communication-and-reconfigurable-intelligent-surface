...
\begin{section}{RIScatter}
	...
	\begin{subsection}{System Model}
		...
		When backscatter symbol tuple $x_{m_\mathcal{K}}$ occurs, the receive signal $y$ follows \gls{cscg} distribution $\mathcal{CN}(0,\sigma_{m_{\mathcal{K}}}^2)$ with variance
		\begin{equation}
		...
		Let $L = M^K$ be the number of backscatter symbol tuples.
		...
		\end{equation}
		where $t_l$ is the decision threshold between hypotheses of tuple $x_{l}$ and $x_{l+1}$.
		...
		\end{equation}
		where $I_{\text{P}}(s;y|x_{\mathcal{K}})$ is the primary information function
		\begin{equation}
		...
		% With a slight abuse of notation, we also define the weighed sum-functions
		% \begin{align}
		% 	I(x_{m_{\mathcal{K}}})
...
\begin{section}{Rate-Region Characterization}
	With a slight abuse of notation, we define the weighed sum mutual information
	\begin{equation}
		I(x_{\mathcal{K}}) = \rho I_{\text{P}}(s;y|x_{\mathcal{K}}) + (1 - \rho) I_{\text{B}}(x_{\mathcal{K}};\hat{x}_{\mathcal{K}}),\label{eq:weighted_sum_mutual_information}
	\end{equation}
	% \begin{align}
	% 	I(x_{\mathcal{K}})
	% 	 & \triangleq \rho I_{\text{P}}(s;y|x_{\mathcal{K}}) + (1 - \rho) I_{\text{B}}(x_{\mathcal{K}};\hat{x}_{\mathcal{K}}),\label{eq:weighted_sum_mutual_information}
	...
	To obtain the achievable primary-(total-)backscatter rate region for the RIScatter network in Fig.~\ref{fg:riscatter_network}, we maximize the weighted sum mutual information with independent encoding at all nodes.\footnote{Joint encoding over multiple nodes formulates an augmented source with larger codebook and can be viewed as its special case.}
	Denote the input distribution of node $k$ as $\boldsymbol{p}_k = [p(x_{1_k}), \ldots, p(x_{M_k})]^\mathsf{T} \in \mathbb{I}^M$.
	The problem is formulated as
	% To characterize the achievable primary-(total-)backscatter rate region for the RIScatter network in Fig.~\ref{fg:riscatter_network}, we aim to maximize the weighted sum mutual information with respect to input distribution $\{\boldsymbol{p}_k\}_{k \in \mathcal{K}}$, active beamforming $\boldsymbol{w}$, and decision thresholds $\boldsymbol{t}$ by
	\begin{maxi!}
	...
	It generalizes conventional \gls{bc} by allowing \gls{csi}- and \gls{qos}-adaptive input distribution and decision region design.
...
\begin{section}{Simulation Results}
	...
	\begin{subsection}{Comparison of Scattering Applications}
		...
		\begin{itemize}
			...
			% which follows non-zero mean complex Gaussian distribution $\mathcal{CN}\bigl((\boldsymbol{h}_{\text{D}}^\mathsf{H} + \sum_{k} \alpha_k \boldsymbol{h}_{\text{C},k}^\mathsf{H} x_{m_k}) \boldsymbol{w},\sigma_v^2\bigr)$ under hypothesis $x_{m_{\mathcal{K}}}$.
			% The corresponding \gls{pdf} of receive energy over $N$ primary blocks is
			% \begin{equation}
			% 	f^{\text{\gls{bbc}}}(z|x_{m_{\mathcal{K}}}) = \frac{(z-\mu_{m_{\mathcal{K}}}^{\text{\gls{bbc}}})^{N-1} \exp \bigl(-(z-\mu_{m_{\mathcal{K}}}^{\text{\gls{bbc}}})/\sigma_v^2 \bigr)}{\sigma_v^{2N} (N-1)!},
			...
			\end{equation}
			The total backscatter rate of coherent decoding approaches $K \log M$ when $N$ is sufficiently large.
			% which only involves noise uncertainty under hypothesis $x_{m_{\mathcal{K}}}$.
		...
		% For \gls{bbc} with coherent energy detection, the conditional \gls{pdf} of receive energy \eqref{eq:energy_distribution_bbc} is more skewed at a large $N$, such that the equivalent \gls{dmmac} \eqref{eq:dmmac} has lower error probabilities.
