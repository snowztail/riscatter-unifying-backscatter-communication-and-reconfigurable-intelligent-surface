...
\begin{abstract}
	...
	% In this paper, we depart from those concepts and introduce UniScatter as a new paradigm to support the high throughput, ultralow power, and ubiquitous connectivity of future wireless networks using shared spectrum, energy, and infrastructures.
	In this paper, we depart from those concepts and introduce UniScatter as a new paradigm to support the high throughput, ultralow power, and ubiquitous connectivity of future wireless networks.
	Instead of treating probability distribution of reflection states as equiprobable (as scattering source of \gls{bc}/\gls{sr}) or degenerate (as reflecting element of \gls{ris}), UniScatter node adapts input distribution based on \gls{csi} and link priority to balance information encoding and channel reconfiguration in a mutualistic and flexible manner.
	% Accommodated to signal characteristics, UniScatter receiver semi-coherently decodes all nodes from accumulated energy, re-encodes to determine the equivalent channel, then decodes the primary link to support backscatter modulation and preserve passive beamforming.
	% Accommodated to signal characteristics, UniScatter receiver semi-coherently decodes all nodes from accumulated energy, combines reflection pattern with \gls{csi} for the equivalent channel, then decodes the primary link to support backscatter modulation and preserve passive beamforming.
	...
	It reduces the complexity of cooperative decoding while preserves the benefit of backscatter modulation and passive beamforming.
	...
	% Using shared spectrum, energy, and infrastructures, each UniScatter node adapts probability distribution of reflection states based on \gls{csi} to balance information encoding and passive beamforming in a mutualistic and flexible manner.
	% The scatter source of \gls{bc}/\gls{sr} and reflecting element of \gls{ris} can be viewed as extreme cases of UniScatter node, where the input distribution boils down to equiprobable and degenerate, respectively.
	% To accommodate signal characteristics, we propose a practical receiver that semi-coherently decodes all nodes from accumulated energy, re-encodes to determine the equivalent channel, then decodes the primary link.
	% It reduces the complexity of cooperative decoding while preserves the benefit of backscatter modulation and passive beamforming.
	...
	Simulation results demonstrate UniScatter can flexibly control the transmit-assist tradeoff via smart input distribution design.


	% reduces the complexity of cooperative decoding while preserves the benefit of passive beamforming.
	% reduces the complexity of cooperative decoding while preserves the benefit of backscatter modulation and passive beamforming.
