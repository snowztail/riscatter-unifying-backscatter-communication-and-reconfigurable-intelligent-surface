...
\begin{section}{Metascatter-Enabled Network}
	...
	\begin{subsection}{Receiving Strategy}
		...
		\end{equation}
		where $x_{m_k}$ and $x_{m_\mathcal{K}}$ are the symbol and symbol set associated with state $m_k$ and state set $m_{\mathcal{K}}$, respectively.%
		\footnote{
		...
		\begin{remark}
			...
			Next, we will constrain the decision regions to convex and consider suboptimal threshold design accordingly.
			% Hence,
			% threshold design papers are constrained to convex quantizer, we may end up using a similar assumption.
			% decision region design to convex

			% design over convex decision regions.

			% decision regions are first restricted to convex

			% The question was answered in \cite{Qian2019b,Nguyen2021b} only for single source with binary inputs.


			% Even for a single source with non-binary inputs,
		...
		% divide the accumulated energy $z$ into disjoint decision regions.
		% where $x_{m_k}$ is the backscatter symbol associated with state $m_k$ and $x_{m_\mathcal{K}}$ is
		% and $x_{m_\mathcal{K}}$ are the backscatter symbols associated with state $m_k$ and $m_{\mathcal{K}}$, respectively.

		% At primary block $n$, the received signal
		...
		% To further reduce decoding complexity, we divide the continuous output $z$ into disjoint decision regions to construct a \gls{dtmac}, where the transition probability from tag input combination $x_{m_{\mathcal{K}}}$ to detector output combination $\hat{x}_{m_{\mathcal{K}}'}$ is
		% \begin{equation}
		% 	P(\hat{x}_{m_{\mathcal{K}}'} \mid x_{m_{\mathcal{K}}}) = \int_{\mathcal{R}_{m_{\mathcal{K}}'}} f(z \mid \mathcal{H}_{m_{\mathcal{K}}}) \dd z,
		% 	\label{eq:dtmac}
		% \end{equation}
