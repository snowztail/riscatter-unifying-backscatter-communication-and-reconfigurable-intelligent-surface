...
\begin{document}
				...
				\subfloat[Block diagram]{
					\resizebox{0.6\columnwidth}{!}{
						\begin{circuitikz}[transform shape]
							...
							\draw (0,1.75)
								to[short] ++(-0.75,0)
								to[short] ++(0,0.75) node[bareantenna](A){};
							\draw (0,1.75)
	...
	\begin{section}{System Model}
		...
		\end{figure}
		% As shown in Fig.~\ref{fi:system}, we propose a single-user (UE) single-tag (TG) symbiotic radio network where the RF signal generated by a $Q$-antenna Access Point (AP) is shared by two coexisting systems. In the primary AP-UE downlink system, the AP broadcasts to the single-antenna user. In the secondary AP-TG-UE backscatter system, the user serves as the reader of $L$ nearby single-antenna tags, while tag $l_k \in \mathcal{L}_k \triangleq \{1,\ldots,L_k\}$ harvests energy from and modulates information over the ambient RF signal by varying its reflection coefficient $\Gamma_{l_k}$\footnote{Due to the product-distance path loss model corresponding to finite-size tag reflection \cite{Ozdogan2020}, the backscatter dyadic channel deteriorates fast when the forward/backward distance increases or the number of hops increases \cite{Griffin2008}. Therefore, we assume each tag only contributes to its nearest user (i.e., creates no interference to other users), and omit all signals backscattered by two or more times.}. Denote the AP-UE $k$ direct channel as $\boldsymbol{h}_{\mathrm{D},k}^H \in \mathbb{C}^{1 \times Q}$, the AP-TG $l_k$ forward channel as $\boldsymbol{h}_{\mathrm{F},{l_k}}^H \in \mathbb{C}^{1 \times Q}$, and the TG $l_k$-UE $k$ backward channel as $h_{\mathrm{B},l_k} \in \mathbb{C}$. We consider the quasi-static block fading model and assume the CSI of all direct, forward and backward channels are known at the AP.\footnote{Due to the lack of RF chains at the passive tags, accurate and efficient estimation for the forward and backward channels can be challenging. Our assumption is based on the following reasoning. First, the direct AP-UE channel and the dyadic AP-TG-UE channel can be estimated with the AP sending pilots, the tags responding in a pre-defined frame-based manner, and the users performing least-square estimation within each frame\cite{Bharadia2015,Yang2015b,Guo2019e}. Second, the UE-TG-UE channel can be estimated similarly with the full-duplex users sending pilots\cite{Mishra2019}. Third, the users seek reciprocity to recover the TG-UE backward channel from the estimated UE-TG-UE channel\cite{Mishra2019b,Zawawi2019}, then extracts the AP-TG forward channel from the estimated AP-TG-UE channel provided by the AP. The whole process can be accelerated by the parallel estimation protocol proposed in \cite{Jin2021}.} Define $s_k$ as the primary symbol for user $k$ with $\boldsymbol{s} \triangleq [s_1,\ldots,s_K]^H \in \mathbb{C}^{K \times 1}$ satisfying $\mathbb{E}\{\boldsymbol{s}\boldsymbol{s}^H\}=\boldsymbol{I}$, and $c_{l_k}$ as the secondary symbol of tag $l_k$ with $\boldsymbol{c}_k \triangleq [c_1,\ldots,c_{L_k}]^H \in \mathbb{C}^{L_k \times 1}$ satisfying $\mathbb{E}\{\boldsymbol{c}_k\boldsymbol{c}_k^H\}=\boldsymbol{I}$. Let $s_k \sim \mathcal{CN} (0,1)$ and $c_{l_k}$ employs $M$-PSK by \eqref{eq:c}. As tags typically transmit at a much lower rate than the AP, we assume the secondary symbol period $T_c$ is $N \in \mathbb{Z}_{++}$ times the primary symbol period $T_s$ (i.e., $T_c=NT_s$) and focus on one particular secondary symbol period. For linear precoding with no dedicated tag beams, the transmit signal during the primary symbol period $n \in \mathcal{N} \triangleq \{1,\ldots,N\}$ is
		% \begin{equation}
