...
\begin{section}{UniScatter}
	...
	\begin{subsection}{System Model}
		...
		\begin{remark}
			Interestingly, the capacity-achieving decision design for \gls{dmtc} remains under-investigated, and some attempts were made for single source with binary inputs \cite{Qian2019b,Nguyen2021b}.
		...
		% In particular, we define a general bijective mapping from backscatter index tuples to ordered indexes $f : m_{\mathcal{K}} \to \mathcal{L} \triangleq \{1,\ldots,L \triangleq M^K\}$, and use both notation interchangably in the following context.
		% In particular, we define a general bijective mapping from backscatter index tuples to ordered indexes $\mathcal{M}^K \to \mathcal{L} \triangleq \{1,\ldots,L \triangleq M^K\}$, $m_{\mathcal{K}} \mapsto l$ and use both notation interchangably in the following context.
		...
		\begin{equation}
			...
			\label{eq:dmtmac}
		\end{equation}
		and backscatter channel coding can be performed on top of it.
	\end{subsection}
	...
	\begin{subsection}{Achievable Rates}
		...
		\begin{equation}
			I_{\mathrm{B}}(x_{m_{\mathcal{K}}};\hat{x}_{\mathcal{K}}) \triangleq \sum_{m_{\mathcal{K}}'} P(\hat{x}_{m_{\mathcal{K}}'}|x_{m_{\mathcal{K}}}) \log \frac{P(\hat{x}_{m_{\mathcal{K}}'}|x_{m_{\mathcal{K}}})}{P(\hat{x}_{m_{\mathcal{K}}'})},
		...
		\begin{equation}
			% I_{\mathrm{P}}(s;y|x_{\mathcal{K}}) = \sum_{m_{\mathcal{K}}} P_{\mathcal{K}}(x_{m_{\mathcal{K}}}) \log \left(1 + \frac{\lvert \boldsymbol{h}_{\mathrm{E}}^H(x_{m_{\mathcal{K}}}) \boldsymbol{w} \rvert^2}{\sigma_v^2}\right).
...
\begin{section}{Input Distribution, Active Beamforming, and Decision Threshold Design}
	...
	\begin{subsection}{Active Beamforming}
		...
		\end{equation}
		is the regularized incomplete Gamma function that substitutes the \gls{dmtmac} transition probability \eqref{eq:dmtmac}.
