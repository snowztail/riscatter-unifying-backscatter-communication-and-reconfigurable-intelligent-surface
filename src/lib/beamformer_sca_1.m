function [beamformer, dmtc, weightedSumRate] = beamformer_sca_(weight, symbolRatio, equivalentChannel, transmitPower, noisePower, equivalentDistribution, threshold, tolerance)
	...
	while ~isConverged
		...
		cvx_solver mosek
			...
			% variables beamformerMatrix(nTxs, nTxs) regularizedGamma(nInputs, nOutputs) epsilonLower(nInputs, nOutputs + 1) epsilonUpper(nInputs, nOutputs + 1) xiLower(nInputs, nOutputs + 1) xiUpper(nInputs, nOutputs + 1);
