...
figure('Name', 'Average Primary-(Sum-)Backscatter Rate Region vs Number of Output Quantization Bits', 'Position', [0, 0, 500, 400]);
object = gobjects(nVariables, 1);
hold all;
for iVariable = 1 : nVariables
	nBins = Variable(iVariable).nBins;
	object(iVariable) = plot(region{iVariable}(1, :) / log(2), 1e3 * region{iVariable}(2, :) / log(2), 'DisplayName', strcat('$b = ', num2str(log2(nBins)), '$'));
end
