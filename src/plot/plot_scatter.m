...
figure('Name', 'Average Primary-(Sum-)Backscatter Rate Region vs Tag Scatter Ratio', 'Position', [0, 0, 500, 400]);
plotHandle = gobjects(nVariables, 1);
hold all;
for iVariable = 1 : nVariables
	scatterRatio = Variable(iVariable).scatterRatio;
	plotHandle(iVariable) = plot(region{iVariable}(1, :) / log(2), region{iVariable}(2, :) / log(2), 'DisplayName', strcat('$\alpha = ', num2str(scatterRatio), '$'));
end
