...
for iVariable = 1 : nVariables
	...
	plotHandle(iVariable) = plot(region{iVariable}(1, :) / log(2), region{iVariable}(2, :) / log(2), 'DisplayName', strcat('$\gamma_B = ', num2str(pow2db(transmitPower) - cascadedPathLoss - pow2db(noisePower)), '$ dB'));
end
