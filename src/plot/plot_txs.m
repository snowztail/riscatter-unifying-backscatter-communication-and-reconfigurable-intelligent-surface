clear; run('../setup'); run(strcat('config_', erase(mfilename, 'plot_'))); clc;
...
figure('Name', 'Average Primary-(Sum-)Backscatter Rate Region vs Number of Transmit Antennas', 'Position', [0, 0, 500, 400]);
object = gobjects(nVariables, 1);
hold all;
for iVariable = 1 : nVariables
	nTxs = Variable(iVariable).nTxs;
	object(iVariable) = plot(region{iVariable}(1, :) / log(2), 1e3 * region{iVariable}(2, :) / log(2), 'DisplayName', strcat('$Q = ', num2str(nTxs), '$'));
end
