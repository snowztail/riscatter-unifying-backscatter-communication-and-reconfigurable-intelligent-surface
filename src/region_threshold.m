...
for iVariable = 1 : nVariables
	...
	for iWeight = 1 : nWeights
		...
		[rate, distribution, threshold, beamforming] = block_coordinate_descent(nTags, symbolRatio, transmitPower, noisePower, nBins, weight, equivalentChannel, 'Distribution', 'kkt', 'Beamforming', 'pgd', 'Threshold', Variable(iVariable).Threshold);
		if any(isnan(rate))
			return
		else
